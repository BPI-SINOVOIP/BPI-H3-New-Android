...
status_t BufferQueue::queueBuffer(int buf,
    ...
    input.deflate(&timestamp, &isAutoTimestamp, &crop, &scalingMode, &transform,
            &async, &fence, &videoAddr0, &videoAddr1, &videoAddr2, &videoAddr3,
            &videoAddr4, &videoAddr5, &videoFormat, &interlace, &topFieldFirst);
    ...
    { // scope for the lock
        ...
        ST_LOGV("queueBuffer: slot=%d/%llu time=%#llx crop=[%d,%d,%d,%d] "
                "tr=%#x scale=%s videoAddr[0x%x 0x%x 0x%x] videoFormat:0x%x"
                "interlace:%d, topFieldFirst:%d, %p %d %d",
                buf, mFrameCounter + 1, timestamp,
                crop.left, crop.top, crop.right, crop.bottom,
                transform, scalingModeName(scalingMode),
                videoAddr0, videoAddr1, videoAddr2, videoFormat,
                interlace, topFieldFirst,
                this, mOverrideMaxBufferCount, mMaxAcquiredBufferCount);

        const sp<GraphicBuffer>& graphicBuffer(mSlots[buf].mGraphicBuffer);
        graphicBuffer->setVideoBufferInfo(videoAddr0, videoAddr1, videoAddr2, videoAddr3, videoAddr4, videoAddr5, videoFormat);
        graphicBuffer->setDitInfo(interlace, topFieldFirst);
...
int BufferQueue::getVideoOutputType()
{
    return 0;
}

status_t BufferQueue::setCompositionType(int display, int type)
{
    return 0;
}
