...
public:
    ...
    virtual status_t setCompositionType(int display, int type) {
        Parcel data, reply;
        data.writeInterfaceToken(IGraphicBufferConsumer::getInterfaceDescriptor());
        data.writeInt32(display);
		data.writeInt32(type);
        status_t result = remote()->transact(SET_COMPOSITION_TYPE, data, &reply);
        if (result != NO_ERROR) {
            return result;
        }
        return reply.readInt32();
        ...
        uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
        ...
        case SET_COMPOSITION_TYPE: {
            ...
            status_t result = setCompositionType(display, type);
            reply->writeInt32(result);
            return NO_ERROR;
        } break;
        case DUMP: {
