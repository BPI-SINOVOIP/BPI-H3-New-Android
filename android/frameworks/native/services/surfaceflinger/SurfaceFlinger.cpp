...
void SurfaceFlinger::bootFinished()
{
    ...
    setDisplayParameter(HWC_DISPLAY_PRIMARY, DISPLAY_CMD_BOOTFINISH, 0, 0, 0);
    // stop boot animation
...
status_t SurfaceFlinger::getDisplayInfo(const sp<IBinder>& display, DisplayInfo* info) {
    ...
    public:
        ...
        static int getBuildDensity()  {
            return getDensityFromProperty("persist.sys.disp_density"); }
    };
                    ...
                    if (disp != NULL) {
                                ...
                                || (state.frame != draw[i].frame))
                        {
                            ...
                            if (state.type == DisplayDevice::DISPLAY_PRIMARY) {
                                ...
                                frame = Rect(getHwComposer().getWidth(HWC_DISPLAY_EXTERNAL),
                                    getHwComposer().getHeight(HWC_DISPLAY_EXTERNAL));
                            }
                            disp->setProjection(orientation, state.viewport, frame);
                        }
                    }
                }
            }
...
void SurfaceFlinger::onInitializeDisplays() {
    ...
    //use the external layer stack.
    sp<IBinder> binder1 = mBuiltinDisplays[DisplayDevice::DISPLAY_EXTERNAL];
    if(binder1 != NULL){
        sp<const DisplayDevice> hw1(getDisplayDevice(binder1));
        if(hw1 != NULL){
            DisplayState d1;
            d1.what = DisplayState::eDisplayProjectionChanged |
                    DisplayState::eLayerStackChanged;
            d1.token = mBuiltinDisplays[DisplayDevice::DISPLAY_EXTERNAL];
            d1.layerStack = 0;
            d1.orientation = DisplayState::eOrientationDefault;
            d1.frame.makeInvalid();
            d1.viewport = Rect(hw_w,hw_h);
            displays.add(d1);
        }
    }
...
int SurfaceFlinger::setDisplayParameter(int displayId, int cmd,
    ...
    for (size_t i=0; i < count; i++){
        ...
        if (buffer != NULL) {
            ...
            //ALOGD(" buffer format %x, %s", format, layer->getName().string());
        }else{
            layer->getBufferQueue()->query(NATIVE_WINDOW_FORMAT,&format);
        }
        //ALOGD("layer->getName=%sm, format=%x",
        //    layer->getName().string(), format);
        //check video Layer
        if(format != HAL_PIXEL_FORMAT_AW_NV12 &&
            format != HAL_PIXEL_FORMAT_AW_MB420 &&
            format != HAL_PIXEL_FORMAT_AW_MB411 &&
            format != HAL_PIXEL_FORMAT_AW_MB422 &&
            format != HAL_PIXEL_FORMAT_AW_YUV_PLANNER420 &&
            format != HAL_PIXEL_FORMAT_AW_YVU_PLANNER420 &&
            format != HAL_PIXEL_FORMAT_AW_NV21 &&
            format != HAL_PIXEL_FORMAT_YV12 &&
            format != HAL_PIXEL_FORMAT_YCrCb_420_SP
            ){
            continue;
        }else{
            ALOGD("find ve layer %s", layer->getName().string());
        }
        switch(cmd){
        case DISPLAY_CMD_SETVIDEOENABLE:
            if(para0 == 0){
                layer->setFlags(layer_state_t::eLayerHidden,
                    layer_state_t::eLayerHidden);
            }else{
                layer->setFlags(0, layer_state_t::eLayerHidden);
            }
            setTransactionFlags(eTransactionNeeded|eTraversalNeeded);
            return 0;
        case DISPLAY_CMD_SETVIDEOPOSITION:
            layer->setPosition(para0, para1);
            setTransactionFlags(eTransactionNeeded|eTraversalNeeded);
            return 0;
        case DISPLAY_CMD_SETVIDEOSIZE:
            layer->setSize(para0, para1);
            setTransactionFlags(eTransactionNeeded|eTraversalNeeded);
            return 0;
        default:
            break;
