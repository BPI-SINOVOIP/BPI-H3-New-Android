...
public class MediaPlayer implements SubtitleController.Listener
{
    ...
    */
	...
	private static final String getSubtitleMimeType(int subCodecFormat){
		switch(subCodecFormat){
			case 0   : return "subtitle/unknown";
			case 256 : return MEDIA_MIMETYPE_TEXT_SUB;
			case 257 : return MEDIA_MIMETYPE_TEXT_IDXSUB;
			case 258 : return "text/pgs";
			case 259 : return "text/divx";

			case 512 : return MEDIA_MIMETYPE_TEXT_TXT;
			case 513 : return MEDIA_MIMETYPE_TEXT_TXT;
			case 514 : return MEDIA_MIMETYPE_TEXT_SSA;
			case 515 : return MEDIA_MIMETYPE_TEXT_SMI;
			case 516 : return "text/srt";
		}
		return null;
	}
	private static final String getAudioMimeType(int audioCodecFormat){
		switch(audioCodecFormat){
			case 0 : return "unknown" ;
			case 1 : return "audio/mp1";
			case 2 : return "audio/mp2";
			case 3 : return "audio/mp3";
			case 4 : return "audio/mpeg_aac_lc";
			case 5 : return "audio/ac3";
			case 6 : return "audio/dts";
			case 7 : return "audio/lpcm_v";
			case 8 : return "audio/lpcm_a";
			case 9 : return "audio/adpcm";
			case 10 : return "audio/pcm";
			case 11 : return "audio/wma_standard";
			case 12: return "audio/flac";
			case 13 : return "audio/ape";
			case 14 : return "audio/ogg";
			case 15 : return "audio/raac";
			case 16 : return "audio/cook";
			case 17 : return "audio/sipr";
			case 18 : return "audio/atrc";
			case 19 : return "audio/amr";
			case 20 : return "audio/ra";
			case 21 : return "audio/ppcm";
			case 22 : return "audio/wma_loss";
			case 23 : return "audio/wma_pro";
			case 24 : return "audio/mp3_pro";
			case 25 : return "audio/alac";
			case -1 : return "audio/ ";
		}
		return null;
	}
    ...
    public void setDataSource(Context context, Uri uri, Map<String, String> headers)
		...
		/* add by Fuqiang. sart   -----------------------------------}} */
		//This is used for getting the right BD video path which can play for liballwinner.
		mIso = new ISOManager(context);
		/* add by Fuqiang. end   -----------------------------------}} */
    ...
    private void setDataSource(String path, String[] keys, String[] values)
		...
		/* add by Fuqiang. sart   -----------------------------------}} */
		//This is used for getting the right BD video path which can play for liballwinner.
        if(path != null && path.contains("/BDMV/STREAM"))
		{
			Log.d(TAG,"It is BD video directoty, make the path !!!");
			setBdFolderPlayMode(true);
			int idx = path.indexOf("/BDMV/STREAM");
			path = path.substring(0, idx);
			path = "bdmv://" + path;
		}
		else if(path != null && path.contains(".iso") && !path.contains("bdmv"))
		{
			Log.d(TAG,"It is BD video iso, make the path !!!");
			setBdFolderPlayMode(true);
			if(mIso != null)
			{
				path = mIso.getVirtualCDRomPath(path);
				path = "bdmv://" + path;
			}
			else
			{
		...
		/* add by Fuqiang. sart   -----------------------------------}} */
		//This is used for searching subtitle and setting to the CedarX for cmcc.
		if(Config.getTargetPlatform(Config.TARGET_BUSINESS_PLATFORM) == Config.PLATFORM_CMCCWASU)
		{
			...
			if (mSrtList != null && mSrtList.size() > 0)
			{
				for (int j = 0; j < mSrtList.size(); j++)
				{
					 ...
					 */
					addTimedTextSource(mSrtList.get(j),
							mMediaTypeList.get(j));
					Log.d("fuqiang", "here j = " + j + ", mSrtList.get(j) = "
    ...
    public void reset() {
        ...
        //make sure all execution finished before powersaving.
        stayAwake(false);
    ...
    static public class TrackInfo implements Parcelable {
        ...
        TrackInfo(Parcel in) {
            ...
            if (mTrackType == MEDIA_TRACK_TYPE_TIMEDTEXT) {
				//add mime type
                int subtitleCodecFormat = in.readInt();//
                String mime = getSubtitleMimeType(subtitleCodecFormat);
                mFormat = MediaFormat.createSubtitleFormat(
                    mime, language);
				 //source-type is unstandard MediaFormat KEY, just use const string here.
                 //add source-type:      1->exernal, 0->inline,
                 int sourceType = in.readInt();
                 mFormat.setInteger("source-type", sourceType);//
				...
				//add  channels, samplerate, bitrate, mimetype
				int channel = in.readInt();
				int sampleRate = in.readInt();
				int bitRate = in.readInt();
				int audioCodecFormat = in.readInt();
				String mime = getAudioMimeType(audioCodecFormat);
				mFormat = MediaFormat.createAudioFormat(mime, sampleRate, channel);
				...
				mFormat.setInteger("is-doubleStream",in.readInt());
				int videoCodecFormat= in.readInt();
				mFormat.setInteger("CodecFormat",videoCodecFormat);
			} else{
                mFormat = new MediaFormat();
                mFormat.setString(MediaFormat.KEY_LANGUAGE, language);
            }
...
//    	                	parcel.setDataPosition(0);
    ...
    */
	...
	/* add by Fuqiang. sart   -----------------------------------}} */
	//ISOManager is used for mount and unmount ISO file when playing BD video. 
	public class ISOManager {
		private Context mContext;
		private File cdromRoot = null;
		...
		public String getVirtualCDRomPath(String isoFile) {
			...
			for (MountInfo info : cdromList) {
				if (info.getISOPath().equals(isoFile)) {
					return info.getMountPath();
				}
			}
		...
		public boolean isVirtualCDRom(String filePath) {
			for (MountInfo info : cdromList) {
				if (info.getMountPath().equals(filePath))
					return true;
			}
			return false;
		}
		...
		public void clear() {
			for (MountInfo info : cdromList) {
				ISOMountManager.umount(info.getMountPath());
			}
			cdromList.clear();
		}
		...
		public String getIsoFile(String cdromFile) {
			for (MountInfo info : cdromList) {
				if (info.getMountPath().equals(cdromFile))
					return info.getISOPath();
			}
			return null;
		}
				...
				if (directory.isDirectory()) {
					...
					for (File f : files) {
						if (f.exists()) {
							...
							//Log.d("fuqiang", "__2Uri___fileName___" + fileName);
							int idx_a = fileName.indexOf(".");
							int idx_b = fileName.lastIndexOf(".");
							int idx_c = fileName.length();
							//Log.d("fuqiang", "__2Uri___idx_a___" + idx_a + "___idx_b___"
							// 		+ idx_b + "___idx_c___" + idx_c);
							if (idx_a > 0 && idx_b > 0 && idx_c > 0) {
								for (; idx_a > 0;) {
									String name1 = fileName.substring(0, idx_a);
									//Log.d("fuqiang", "__2Uri___name1___" + name1);
									String name2 = fileName.substring(idx_b, idx_c);
									//Log.d("fuqiang", "__2Uri___name2___" + name2);
									//Log.d("fuqiang", "__2Uri___SUB_EXTS.length___"	+ SUB_EXTS.length);
									if (name1.equals(name)) {
										for (int i = 0; i < SUB_EXTS.length; i++) {
											if (name2.toLowerCase().equals(
												...
												Log.d("fuqiang","get it! filename = " + folder + "/" + fileName);
											}
										}
									}
									idx_a = fileName.indexOf(".", idx_a + 1);
									//Log.d("fuqiang", "__2Uri___idx_a___" + idx_a);
								}
							}
						}
					}
