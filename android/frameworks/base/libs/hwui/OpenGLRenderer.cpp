...
void OpenGLRenderer::setupDrawColor(float r, float g, float b, float a) {
	...
	/*
	 * In later version of Android4.2.2, some views of the mobile's launch apk
	 * will flick. Temporarily bypass the problem.
	 */
	char value[PROPERTY_VALUE_MAX];
	property_get("ro.build.product", value, "0");
	if (strstr(value, "wasu")) {
		const float EPS = 0.0000001;
	    float alpha = a - 0.509804;
	    if( fabs(alpha) <= fabs(EPS) ){
	        mSetShaderColor = mDescription.setColor(r, g, b,1);
	        return;
	    }
	}
