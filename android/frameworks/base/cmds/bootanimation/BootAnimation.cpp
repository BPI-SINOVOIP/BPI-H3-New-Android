...
BootAnimation::BootAnimation() : Thread(false)
{
    ...
    if(access(USER_BOOTVIDEO_FILE, R_OK) == 0){
        mBootVideo = true;
        mVideoFile = USER_BOOTVIDEO_FILE;
    } else if (access(SYSTEM_BOOTVIDEO_FILE, R_OK) == 0){
        mBootVideo = true;
        mVideoFile = SYSTEM_BOOTVIDEO_FILE;
...
void BootAnimation::startBootMedia(){
    if (mBootVideo) {
        ...
        leftVolume = rightVolume = get_system_volume();

        mPlayer->setDataSource(mVideoFile, NULL);
...
status_t BootAnimation::readyToRun() {
	...
	if(mBootVideo)
	{
        ...
        int index;
        AudioSystem::getStreamVolumeIndex(AUDIO_STREAM_MUSIC, &index, AUDIO_DEVICE_OUT_DEFAULT);
        ...
        do {
            ...
            if (isIndexChange == 0) {
                int d;
                AudioSystem::getStreamVolumeIndex(AUDIO_STREAM_MUSIC, &d, AUDIO_DEVICE_OUT_DEFAULT);
                if (d != index) { // reset to full volume when AudioSystem ready
                    isIndexChange = true;
...
void BootAnimation::checkExit() {
	...
	{
		// do not check exit, make sure play all the pictures
		//return;
	}
...
bool BootAnimation::movie()
{
    ...
    for (;;) {
        ...
        if (sscanf(l, "%d %d %d", &width, &height, &fps) == 3) {
			...
			if(getBusinessPlatform() == PLATFORM_CMCCWASU && count == 0 && pause == 0)
			{
				ALOGD("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX boot animation!!!!");
				//part.count = 1;
            	//part.pause = 15;
			}
