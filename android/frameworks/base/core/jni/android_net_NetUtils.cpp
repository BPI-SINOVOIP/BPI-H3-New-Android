...
int start_pppoe(const char * const phyif,
...
int get_pppoe_status(const char *ifname);
        ...
        jstring username,jobject dhcpResults)
{
    ...
    const char *phyifStr = env->GetStringUTFChars(ifname, NULL);
    if (phyifStr == NULL) return (jboolean)false;

    const char *usernameStr = env->GetStringUTFChars(username, NULL);
    if (usernameStr == NULL) return (jboolean)false;

    result = ::start_pppoe(phyifStr, usernameStr, pppif, ipaddr, gateway, &prefixLength,
            dns, server, &lease, vendorInfo, domains, mtu);

    if (result != 0) {
        ALOGD("start_pppoe failed : %s", phyifStr);
    }

    env->ReleaseStringUTFChars(ifname, phyifStr);
    ...
    if (result == 0) {
        ...
        // set mIfaceName
        // dhcpResults->setInterfaceName(ifname)
        env->CallVoidMethod(dhcpResults, dhcpResultsFieldIds.setInterfaceName, env->NewStringUTF(pppif));
...
static jint android_net_utils_pppoeStatus(JNIEnv* env, jobject clazz, jstring ifname)
{
    int result;

    const char *nameStr = env->GetStringUTFChars(ifname, NULL);
    result = ::get_pppoe_status(nameStr);
    env->ReleaseStringUTFChars(ifname, nameStr);
    return (jint) result;
}
...
static JNINativeMethod gNetworkUtilMethods[] = {
    ...
    { "pppoeStatus", "(Ljava/lang/String;)I",  (void *)android_net_utils_pppoeStatus },
