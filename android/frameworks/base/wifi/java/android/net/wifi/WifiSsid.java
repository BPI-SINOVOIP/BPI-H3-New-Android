...
public class WifiSsid implements Parcelable {
    ...
    private void convertToBytes(String asciiEncoded) {
        ...
        // chinese ap support
        // shanxiaoxi@allwinnertech.com
        // Store the origin ascii string
        mAsciiStr = asciiEncoded;
        // end
    ...
    public String toString() {
        ...
        out.flip();
        // =====================shanxiaoxi@allwinnertech.com==================
        // If convert as utf-8 error, try gbk again
        if (result.isError()) {
            if (DBG)
            ...
            out = CharBuffer.allocate(32);

            result = decoder.decode(ByteBuffer.wrap(ssidBytes), out, true);
            out.flip();

            if (DBG) {
                // try decode as GBK, and print it byte
                for (int i = 0; i < ssidBytes.length; i++) {
                    Log.d(TAG, "Arigin bytes ["  + i + "] = " + Integer.toHexString((int)ssidBytes[i]));
                }
            }
            try {
                ssidBytes = out.toString().getBytes("GBK");
            } catch (Exception ex) {
                Log.d(TAG, ex.getMessage());
            }
            if (DBG) {
                for (int i = 0; i < ssidBytes.length; i++) {
                    Log.d(TAG, "After bytes ["  + i + "] = " + Integer.toHexString((int)ssidBytes[i]));
                }
            }
        }
        // ==========================end===========================================
