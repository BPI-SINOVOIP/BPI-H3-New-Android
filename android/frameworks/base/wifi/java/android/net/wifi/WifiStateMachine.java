...
public class WifiStateMachine extends StateMachine {
   ...
   /* Set the frequency band */
        ...
        if (Settings.Secure.getInt(cr, Settings.Secure.WIFI_CHEAT_ON, 0) != 0) {
            if(mNetworkInfo != null && !mNetworkInfo.isConnected()) {
                ...
                if(networkinfo != null && networkinfo.isConnected()) {
                    ...
                    log("syncRequestConnectionInfo: WifiInfo = " + notwifiInfo);
                    return notwifiInfo;
                }
            }
        }
    ...
    private boolean isValidAscii(WifiConfiguration config) {
        // =====================shanxiaoxi 2013-12-16==================
        // chinese ap support
        if (DBG) {
            log("Save network!!");
            log("Config SSID: " + config.SSID);
        }
        String tmpSSID = config.SSID.replace("\"", "");
        if (DBG) {
            log("Config trim SSID: " + tmpSSID);
        }
        for (ScanResult tmpResult : mScanResults) {
            if (DBG)
                log("Result SSID: " + tmpResult.SSID);
            if (tmpResult.SSID.equals(tmpSSID) && !tmpResult.wifiSsid.isValidAscii) {
                if (DBG)
                    log("SSID: " + config.SSID + " is GBK set!!!");
                config.originSsid = tmpResult.wifiSsid.mAsciiStr;
                return false;
            }
        }
        return true;
        ...
        public boolean processMessage(Message message) {
            switch(message.what) {
                ...
                case CMD_STOP_DRIVER:
                    ...
                    mInDelayedStop = true;
                    mDelayedStopCounter++;
                    if (true) log("Delayed stop message " + mDelayedStopCounter);
            ...
            switch(message.what) {
                ...
                case WifiManager.CONNECT_NETWORK:
                    ...
                    if (config != null) {
                        // ================shanxiaoxi 2013-12-16============
                        // chinese ap support
                        if (!isValidAscii(config))
                            config.isValidAscii = false;
                        // ================shanxiaoxi end==================
