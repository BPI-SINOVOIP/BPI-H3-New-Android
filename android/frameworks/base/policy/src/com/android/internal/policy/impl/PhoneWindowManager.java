...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    static final int LONG_PRESS_POWER_SET_720P = 4;

    static final int SHORT_PRESS_POWER_SLEEP = 0;
    static final int SHORT_PRESS_POWER_SHUT_OFF = 1;
    static final int SHORT_PRESS_POWER_SHUT_OFF_NO_CONFIRM = 2;
    static final int SHORT_PRESS_POWER_SUPER_STAND_BY = 3;
    ...
    static {
        ...
        if (Config.getTargetPlatform(Config.TARGET_BUSINESS_PLATFORM) != Config.PLATFORM_IPTV) {
            sApplicationLaunchKeyCategories.append(
    ...
    private List<Integer> mDispAvFormat;
    private String[] mDispAvFormatString;
    private List<Integer> mDispHdmiFormat;
    private String[] mDispHdmiFormatString;
    private int mDispOutput = 0;
    private int dispOutputOffset = 0;
    private Toast dispOutputSwitchToast;

    Runnable mDisplaySwitchToastRunnable = new Runnable(){
        public void run() {
                mHandler.removeCallbacks(mDisplayOutputSwitchRunnable);
                if (dispOutputSwitchToast == null)
                    dispOutputSwitchToast=Toast.makeText(mContext, "", Toast.LENGTH_SHORT);
                dispOutputOffset++;
                if (mHdmiPlugged)
                {
                    int pos = mDispHdmiFormat.indexOf(mDispOutput);
                    int strpos=(pos+dispOutputOffset)%mDispHdmiFormatString.length;
                    dispOutputSwitchToast.setText(mDispHdmiFormatString[(pos+dispOutputOffset)%mDispHdmiFormatString.length]);
                    dispOutputSwitchToast.show();
                }
                else
                {
                    int pos = mDispAvFormat.indexOf(mDispOutput);
                    dispOutputSwitchToast.setText(mDispAvFormatString[(pos+dispOutputOffset)%mDispAvFormatString.length]);
                    dispOutputSwitchToast.show();
                }
                mHandler.postDelayed(mDisplayOutputSwitchRunnable,2500);
        }
    };
    Runnable mDisplayOutputSwitchRunnable = new Runnable(){
        public void run() {
            if (mHdmiPlugged)
            {
                int pos = mDispHdmiFormat.indexOf(mDispOutput);
                int nextOutput=mDispHdmiFormat.get((pos+dispOutputOffset)%mDispHdmiFormat.size());
                dispOutputOffset=-1;
                if (nextOutput != mDispOutput)
                {
                    mDm.setDisplayOutput(Display.TYPE_BUILT_IN,nextOutput);
                }
            }
            else
            {
                int pos = mDispAvFormat.indexOf(mDispOutput);
                int nextOutput=mDispAvFormat.get((pos+dispOutputOffset)%mDispAvFormat.size());
                dispOutputOffset=-1;
                if (nextOutput != mDispOutput)
                {
                    mDm.setDisplayOutput(Display.TYPE_BUILT_IN,nextOutput);
    ...
    private UEventObserver mCVBSObserver = new UEventObserver() {
        @Override
        public void onUEvent(UEventObserver.UEvent event) {
            boolean cvbsPlugged = "1".equals(event.get("SWITCH_STATE"));
            setCvbsPlugged(cvbsPlugged);
        }
    };
    ...
    private final Runnable mPowerShortPress = new Runnable() {
        ...
        public void run() {
             ...
             {
                case SHORT_PRESS_POWER_SHUT_OFF:
                    mWindowManagerFuncs.shutdown(true);
                    break;
                case SHORT_PRESS_POWER_SHUT_OFF_NO_CONFIRM:
                    mWindowManagerFuncs.shutdown(false);
                    break;
                case SHORT_PRESS_POWER_SUPER_STAND_BY:
    ...
    private final Runnable mPowerLongPress = new Runnable() {
        ...
        public void run() {
            ...
            case LONG_PRESS_POWER_SHUT_OFF_NO_CONFIRM:
                ...
                /* modified by Gary. end   -----------------------------------}} */
                break;
            case LONG_PRESS_POWER_SET_720P:
                mPowerKeyHandled = true;
                if (mDm != null) {
                    int dispFormat = 0;
                    int type = 0;
                    ...
                    for (int i = 0; i < defFormats.length; i++) {
                        ...
                        type = mDm.getDisplayTypeFromFormat(dispFormat);
                        if(DisplayManager.DISPLAY_OUTPUT_TYPE_HDMI == type)
                            break;
                        dispFormat = 0;
                    }
					//Log.d(TAG,"xxx DisplayOutputType = " + mDm.getDisplayOutputType(Display.TYPE_BUILT_IN));
                    if(0 != dispFormat && DisplayManager.DISPLAY_OUTPUT_TYPE_HDMI == mDm.getDisplayOutputType(Display.TYPE_BUILT_IN))
                    {
                        mDm.setDisplayOutput(Display.TYPE_BUILT_IN, dispFormat);
                    }
            		if(DisplayManager.DISPLAY_OUTPUT_TYPE_HDMI == mDm.getDisplayOutputType(Display.TYPE_BUILT_IN))
					{
            			ActivityManager am = (ActivityManager)mContext.getSystemService(Context.ACTIVITY_SERVICE);
						ComponentName cn = am.getRunningTasks(1).get(0).topActivity;
						String packageName = cn.getPackageName();
						if(packageName.equals("net.sunniwell.app.swsettings.chinamobile"))
						{
						...
						Toast mToast = Toast.makeText(mContext, com.android.internal.R.string.long_press_power_set_720p, Toast.LENGTH_LONG);
						mToast.setGravity(Gravity.CENTER, 0, 0);
						mToast.show();
					}
					else
					{
						Toast mToast = Toast.makeText(mContext, com.android.internal.R.string.long_press_power_set_720p_av, Toast.LENGTH_LONG);
						mToast.setGravity(Gravity.CENTER, 0, 0);
						mToast.show();
					}
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        mDm = (DisplayManager) mContext.getSystemService(Context.DISPLAY_SERVICE);
        ...
        String hdmiFormats[] = mContext.getResources().getStringArray(
        com.android.internal.R.array.support_tv_format_values_hdmi);
        mDispHdmiFormat = new ArrayList<Integer>();
        for (int i = 0; i < hdmiFormats.length; i++) {
            mDispHdmiFormat.add(Integer.valueOf(hdmiFormats[i], 16));
        }
        mDispHdmiFormatString= mContext.getResources().getStringArray(
        com.android.internal.R.array.support_tv_format_entries_hdmi);

        String avFormats[] = mContext.getResources().getStringArray(
        com.android.internal.R.array.support_tv_format_values_av);
        mDispAvFormat = new ArrayList<Integer>();
        for (int i = 0; i < avFormats.length; i++) {
            mDispAvFormat.add(Integer.valueOf(avFormats[i], 16));
        }
        mDispAvFormatString= mContext.getResources().getStringArray(
        com.android.internal.R.array.support_tv_format_entries_av);
        ...
        mDisplayEnhanceModeStrArray = mContext.getResources().getStringArray(
        com.android.internal.R.array.display2_enhance_mode_names);
    ...
    private long _interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
		...
		if(tattrs != null && isFactoryConfigExist(tattrs.flags)){
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            ...
            if (!down) {
                if (mDm==null )
                    return -1;
                mDispOutput = mDm.getDisplayOutput(Display.TYPE_BUILT_IN);
                mHandler.post(mDisplaySwitchToastRunnable);
            }
            return -1;
        ...
        /* support the shortcut keys "setup", "expand" and "browser" */
            ...
            if(!down){
                Intent intent;
                if (Config.getTargetPlatform(Config.TARGET_BUSINESS_PLATFORM) == Config.PLATFORM_CMCCWASU)
                {
        ...
        /* support shortcut keys with color keys for some specific websites and apps */
            ...
            if(!down){
				...
				{
					//add by fuqiang for cmcc dual launcher support
					Log.d(TAG,"green key down!");
					Intent intent = new Intent(Intent.ACTION_MAIN, null);
	                intent.addCategory(Intent.CATEGORY_HOME);
	                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
					Intent choser = Intent.createChooser(intent,null);
					choser.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
					if (intent.resolveActivity(mContext.getPackageManager())!=null)
					{
						mContext.startActivity(choser);
					}
				}
                else
                {
    ...
    void setCvbsPlugged(boolean plugged) {
        if (mCvbsPlugged != plugged) {
            mDmPolicy.notifyDisplayDevicePlugedChanged(DisplayManager.DISPLAY_OUTPUT_TYPE_TV, plugged);
            mCvbsPlugged = plugged;
            //updateRotation(true, true);
           /* fixme: need broadcaset for cvbs plugstate changed ?
            Intent intent = new Intent(Intent.ACTION_HDMISTATUS_CHANGED);
            intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
            intent.putExtra(EXTRA_HDMI_PLUGGED_STATE, plugged);
            intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY
    ...
    void handleMute(int keyCode){
        ...
        try {
			...
			int lastAudibleVolume = audioService.getLastAudibleStreamVolume(AudioManager.STREAM_MUSIC);
			switch (keyCode) {
				case KeyEvent.KEYCODE_VOLUME_DOWN:
					if (lastAudibleVolume > 1)
						audioService.setStreamMute(AudioManager.STREAM_MUSIC | AudioManager.SHOW_MUTE_UI, mIsMute = !mIsMute, mCallback);
					break;
				...
				case KeyEvent.KEYCODE_MUTE:
					if (mIsMute && (lastAudibleVolume == 0)) {
						audioService.adjustSuggestedStreamVolume(AudioManager.ADJUST_RAISE,
							AudioManager.STREAM_MUSIC, AudioManager.FLAG_SHOW_UI | AudioManager.FLAG_VIBRATE, mContext.getPackageName());
					} else if (!mIsMute && (lastAudibleVolume == 1)) {
						audioService.adjustSuggestedStreamVolume(AudioManager.ADJUST_LOWER,
							AudioManager.STREAM_MUSIC, AudioManager.FLAG_SHOW_UI | AudioManager.FLAG_VIBRATE, mContext.getPackageName());
					} else {
						audioService.setStreamMute(AudioManager.STREAM_MUSIC | AudioManager.SHOW_MUTE_UI, mIsMute = !mIsMute, mCallback);
					}
...
/*************************************start****************************************/
	//add for cmccwasu by fuqiang
	private boolean startActivitySave (Intent intent) {
        try {
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            mContext.startActivity(intent);
        } catch (Exception e2) {
...
/*************************************end****************************************/
    ...
    void startDockOrHome() {
		...
		{
			Intent intent = new Intent();
			intent.setClassName("android.app.chinamobile.ott.itv",
					"android.app.chinamobile.ott.itv.ui.activity.InitActivity");
		        String initWizardStr = Settings.Secure.getString(mContext.getContentResolver(), "initialization_wizard");
		        boolean login = SystemProperties.getInt("epg.login", -1)  == 1;
		        boolean needInitWizard = (initWizardStr == null || initWizardStr.equals("0"));
			boolean success = !needInitWizard && login && (/*startActivitySave(new Intent(
					"cn.cmvideo.action.sanping"))
					|| */startActivitySave(new Intent("cn.cmvideo.action.ITV"))
		                        || startActivitySave(intent));
		        Slog.d(TAG, "startDockOrHome for spetial success=" + success);
			if (!success) {
