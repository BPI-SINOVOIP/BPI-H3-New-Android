            ...
            protected void chooseUpstreamType(boolean tryCell) {
                ...
                synchronized (mPublicSync) {
                    ...
                    for (Integer netType : mUpstreamIfaceTypes) {
                        ...
                        NetworkInfo eth_info = null;
                        try {
                            info = mConnService.getNetworkInfo(netType.intValue());
                            if (wifi_cheat_on != 0)
                                eth_info = mConnService.getNetworkInfo(ConnectivityManager.TYPE_ETHERNET);
                        } catch (RemoteException e) { }
                        if ((info != null) && info.isConnected()) {
                            upType = netType.intValue();
                            // eth cheated wifi, when Tethering choose upstream, need distinguish wifi&&eth
                            if (wifi_cheat_on != 0) { 
                                if((upType == ConnectivityManager.TYPE_WIFI) && 
                                        (eth_info != null) && (eth_info.isConnected()))
