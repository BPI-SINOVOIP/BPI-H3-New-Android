...
final class WifiSettingsStore {
	...
	// softap save v1.0
	private static final int WIFI_AP_ENABLED                    = 4;
	private static final int WIFI_AP_DISABLED                   = 5;
	...
	synchronized boolean isWifiAPToggleEnabled() {
        ...
        if (mAirplaneModeOn) {
            return mPersistWifiAPState == WIFI_AP_ENABLED_AIRPLANE_OVERRIDE;
        } else {
	...
	synchronized boolean handleWifiAPToggled(boolean wifiAPEnabled) {
        // Can Wi-Fi AP be toggled in airplane mode ?
        if (mAirplaneModeOn && !isAirplaneToggleable()) {
            return false;
        }
	...
	// softap save v1.0
	synchronized void setttingsStoreSetWifiSaveState(int state) {
		final ContentResolver cr = mContext.getContentResolver();
