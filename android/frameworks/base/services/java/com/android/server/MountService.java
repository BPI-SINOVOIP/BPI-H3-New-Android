...
class MountService extends IMountService.Stub
	...
	private static final String USB_PATH = "/mnt/usbhost";
	private String usbpath = null;
	private String diagnosePath = null;
	...
	public class ApkInstallListener extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            intent.getAction().equals(Intent.ACTION_PACKAGE_CHANGED)){
               ...
               if(packageName.equals("com.moretv.android")){
                  startAPKFile();
               }
            }
        }
...
/**************************************end***************************************/
    ...
    private final HashMap<String, Integer> mVolumeRefs = Maps.newHashMap();
            ...
            public void run() {
                ...
                try {
                    ...
                    for (String volstr : vols) {
                        ...
                        {
                            synchronized (mVolumesLock) {
                            Slog.e(TAG, "********create volume:"+path);
                             final StorageVolume newVolume = new StorageVolume(new File(path),
                                    com.android.internal.R.string.storage_usb, false, true, false, 0,
                                    false, 0, null);
                            addVolumeLocked(newVolume);
                            mVolumeStates.put(path, state);
                            }
                        }
    ...
    public boolean onEvent(int code, String raw, String[] cooked) {
        ...
        if (code == VoldResponseCode.VolumeStateChange) {
             ...
             */
            Slog.e(TAG,"*****VolumeStateChange:"+cooked[2]+" "+cooked[3]+" "+cooked[7]+" "+cooked[10]);
            ...
            //create new StorageVolume

            if (code == VoldResponseCode.VolumeDiskInserted) {
                if (mVolumesByPath.get(path)==null)
                {
                synchronized (mVolumesLock) {
                    Slog.e(TAG, "********create volume:"+path);
                    ...
                    mVolumeStates.put(path, Environment.MEDIA_UNMOUNTED);
                    mVolumeRefs.put(path,Integer.valueOf(1));
                }
                }
                else
                {
                    if (mVolumeRefs.containsKey(path))
                    {
                        mVolumeRefs.put(path,Integer.valueOf(mVolumeRefs.get(path).intValue()+1));
                    }
                }
            }
            ...
            if (code == VoldResponseCode.VolumeDiskRemoved || 
                code == VoldResponseCode.VolumeBadRemoval) {
                if (mVolumeRefs.containsKey(path))
                {
                    mVolumeRefs.put(path,Integer.valueOf(mVolumeRefs.get(path).intValue()-1));
                    if (mVolumeRefs.get(path).intValue()==0)
                    {
                        Slog.e(TAG, "**********remove volume:"+volume.getPath());
                        synchronized (mVolumesLock) {
                            if (volume!=null)
                                removeVolumeLocked(volume);
                        }
...
/*************************************start****************************************/
	...
	private void installAPKFile(String filename) {
        try{
            Runtime.getRuntime().exec(new String[]{ "pm", "install", filename});
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    private void startAPKFile(){
        Intent intent = new Intent();
        ComponentName componentName =new ComponentName("com.moretv.android","com.moretv.android.StartActivity");
    ...
    private void checkAndStartAPK(){
        if(usbpath == null)
            return;
        String diagnosePath = usbpath + "/diagnose";  //diagnose folder with moretv_xxx.apk
        String moretv_apks[];
        String postfix_name = null;
        File diagnoseFile = new File(diagnosePath);
        String apkFileName = null;
		PackageInfo packageInfo;
        if(diagnoseFile.exists()){
            //get all apk files in diagnose
            moretv_apks = diagnoseFile.list();
            //start all apks in diagnose
            for(int i=0; i<moretv_apks.length; i++){
               postfix_name = moretv_apks[i].substring(moretv_apks[i].lastIndexOf(".")+1);
               if("apk".equals(postfix_name) && moretv_apks[i].contains("moretv")){
                   apkFileName = diagnoseFile + "/" + moretv_apks[i];
                   try {
                       packageInfo = mContext.getPackageManager().getPackageInfo("com.moretv.android", 0);
                   } catch (NameNotFoundException e) {
                       packageInfo = null;
                       e.printStackTrace();
                   }
                   if(packageInfo != null){
                       startAPKFile(); //installed,dont neet to wait for installing
                   }else {
                       installAPKFile(apkFileName);
                   }
               }
            }
        }
...
/*************************************end****************************************/
    ...
    private void notifyVolumeStateChange(String label, String path, int oldState, int newState) {
        ...
        if (action != null) {
            // add for usb recovery
            usbpath = volume.getPath();
			diagnosePath = usbpath + "/diagnose"; //should be "/diagnose/xxxx.apk",match with apk
            upgradePath = usbpath + "/upgrade"; //should be "/upgrade/update.zip",match with apk
            Log.d("CMCC-WASU","++ usbpath = " + usbpath + ", upgradePath = " + upgradePath + ", diagnosePath = " + diagnosePath);
			if(Config.getTargetPlatform(Config.TARGET_BUSINESS_PLATFORM) == Config.PLATFORM_CMCCWASU
					...
					&& USB_PATH.equals(usbpath.substring(0,USB_PATH.length())))
			{
				if(new File(upgradePath).exists() && new File(diagnosePath).exists())
				{
					Log.d("CMCC-WASU","++ there is upgrade & diagnose folder!!");
					final String _action  = action;
					AlertDialog alertDialog = new AlertDialog.Builder(mContext).
						setTitle(com.android.internal.R.string.usb_recovery_title).
						setMessage(com.android.internal.R.string.usb_recovery_message).
						setPositiveButton(com.android.internal.R.string.gpsVerifYes, new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog, int which) {
	                            // TODO Auto-generated method stub
	                            sendStorageIntent(_action, volume, UserHandle.ALL);
	                        }
						}).
						setNegativeButton(com.android.internal.R.string.gpsVerifNo, new DialogInterface.OnClickListener() {
	                        ...
	                        public void onClick(DialogInterface dialog, int which) {
	                            // TODO Auto-generated method stub
	                            Slog.e(TAG, "Cancel Usb Recovery");
	                            checkAndStartAPK();
	                        }
	                    }).
	                    create();
				...
				else if(!(new File(upgradePath).exists()) && new File(diagnosePath).exists())
				{
					Log.d("CMCC-WASU","++ there is only diagnose folder!!");
					checkAndStartAPK();
				}
				else if(new File(upgradePath).exists() && !(new File(diagnosePath).exists()))
				{
					Log.d("CMCC-WASU","++ there is only upgrade folder!!");
					final String _action  = action;
					sendStorageIntent(_action, volume, UserHandle.ALL);
				}
				else
				{
					Log.d("CMCC-WASU","++ there is no useful folder!!");
				}
			}
			else
			{
    ...
    private void updateVoldVolume() {
        mHandler.postDelayed(new Runnable() {
        public void run() {
            try {
                mConnector.execute("volume", "updateVolume");
            } catch (NativeDaemonConnectorException e) {
                /* do nothing */
            }
        }
    ...
    private void removeVolumeLocked(StorageVolume volume) {
        ...
        if (mVolumeRefs.containsKey(volume.getPath()))
            mVolumeRefs.remove(volume.getPath());
    }
    ...
    public MountService(Context context) {
		...
		if(Config.getTargetPlatform(Config.TARGET_BUSINESS_PLATFORM) == Config.PLATFORM_CMCCWASU)
		{
	        ...
	        mContext.registerReceiver(new ApkInstallListener(),filter);
		}
    }
