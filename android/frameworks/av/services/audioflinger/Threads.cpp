...
// TODO: use native method, instead of jni
/*
int requestPriority_native(int pid, int tid, int prio) {
	ALOGD("requestPriority(pid=%d, tid=%d, prio=%d)", pid , tid , prio);

	SchedPolicy sp = (SchedPolicy) getpid() == pid ? SP_AUDIO_SYS : SP_AUDIO_APP;
	int res = set_sched_policy(tid, sp);
	if (res != NO_ERROR) {
		ALOGD("set_sched_policy failed");
		return -res;
	}
	...
	if (res) {
		ALOGD("sched_setscheduler failed, err=%s", strerror(errno));
		return errno;
	}
#else
	return -ENOSYS;
...
void AudioFlinger::PlaybackThread::onFirstRef()
{
    ...
    pid_t tid = this->getTid();

#if (SW_CHIP_PLATFORM == A80_CHIP_PLATFORM)
    ...
    if (err != 0) {
        ...
        ALOGV("Policy SCHED_FIFO priority %d is available for pid %d tid %d; error %d",
            kPriorityFastMixer, getpid_cached, tid, err);
    }
#endif
}
