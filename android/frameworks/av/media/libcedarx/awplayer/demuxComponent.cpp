...
static int DemuxCompSetCacheTime(DemuxCompContext* demux, int nCacheTimeMs)
{
    ...
    {
    if(nMaxBufferSize < demux->nCacheBufferSize)      //* 20MB.
        nMaxBufferSize = demux->nCacheBufferSize;
    }
            ...
            if(demux->pParser != NULL)
            {
                ...
                else
                {
                    if(CdxParserSeekTo(demux->pParser, ((int64_t)nSeekTimeMs)*1000) >= 0)
                    {
                        ...
                        if(demux->pParser->type == CDX_PARSER_HLS)
                        {
                            cdx_int64 hlsDuration =0;
                            CdxParserControl(demux->pParser,CDX_PSR_CMD_SET_HLS_DURATION,(void*)&hlsDuration);
                            nFinalSeekTimeMs = hlsDuration/1000;
                        }
