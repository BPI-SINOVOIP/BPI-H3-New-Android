...
//-------------------------------------------------
// add by fuqiang for cmcc wasu
// 20150105
static long mStartTime = 0;   //from url of cmcc
static int mFirstStart = 0;
// add end
...
//		p->switchTrack(mTrackIndex);
		...
		if (strstr(url, "starttime")!=NULL && strstr(url, "livemode") == NULL)
		{    
				ALOGD("--------------------------- get start time for cmcc wasu ---------------------------");

				char *starttime = strstr(url, "starttime");
				char startTmp[1024] ;
				memset(startTmp, 0, 1024);
				strncpy(startTmp, starttime+strlen("starttime="),strlen(starttime)+1-strlen("starttime="));

				for (int i=0; i<strlen(startTmp); i++) 
				{    
						if (startTmp[i] == '&') 
						{    
								startTmp[i] = '\0';
						...
						mStartTime = (startTmp[i] - '0') + mStartTime*10;
				}
...
status_t MediaPlayerService::Client::start()
{
	...
	// add by buqiang for cmcc
	// 20150105
	if (mStartTime > 0 && mFirstStart == 1)
	{
		ALOGD("start seekTo %d for cmccwasu", mStartTime);
		p->seekTo(mStartTime);
		mFirstStart = 0;
	}
...
status_t MediaPlayerService::Client::getCurrentPosition(int *msec)
{
    ...
    if (ret == NO_ERROR) {
		if (mStartTime > 0)
		{
			ALOGD("xxxxxxxxxxxxxxx [%d] getCurrentPosition = %d", mConnId, *msec);
			mStartTime = *msec;
		}
