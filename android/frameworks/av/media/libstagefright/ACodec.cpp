...
// MARLIN_USE_SECUREOS defined in Android.mk
#if MARLIN_USE_SECUREOS
#define MARLIN_SECURE_PATH 1
#else
#define MARLIN_SECURE_PATH 0
#endif
...
ACodec::~ACodec() {
    if (mSecureDecrypter) {
        if (mDecrypter) {
            WSBSecure_AP4_CencSingleSampleDecrypter_Destroy(mDecrypter);
        }
        if (mBlockCipherFactory) {
            WSBSecure_SkbAp4BlockCipherFactory_Destroy(mBlockCipherFactory);
        }
    } else {
        delete mDecrypter;
...
status_t ACodec::configureOutputBuffersFromNativeWindow(
    ...
    err = native_window_set_usage(
            mNativeWindow.get(),
            usage | GRALLOC_USAGE_HW_TEXTURE | GRALLOC_USAGE_EXTERNAL_DISP | GRALLOC_USAGE_SW_WRITE_OFTEN);
...
static const struct VideoCodingMapEntry {
    ...
    { MEDIA_MIMETYPE_VIDEO_MJPGE, OMX_VIDEO_CodingMJPEG},
...
bool ACodec::DecryptBuffer(sp<ABuffer> buffer, sp<MetaData> crypto, bool video, uint8_t*& secure_out)
{
	uint32_t type;
	const void *key, *iv, *enc_sizes, *plain_sizes;
	uint32_t key_len, iv_len, enc_sizes_len, plain_sizes_len;

	bool is_secure_path = (MARLIN_SECURE_PATH == 1)? true : false;
	if (!video)
	...
	if (is_secure_path) {
		mSecureDecrypter = true;
	}

	if (mBlockCipherFactory == NULL) {
		if (is_secure_path) {
			CHECK_EQ(WSBSecure_SkbAp4BlockCipherFactory_Create(mBlockCipherFactory), WSB_SUCCESS);
		} else {
			CHECK_EQ(WSB_SkbAp4BlockCipherFactory_Create(mBlockCipherFactory), WSB_SUCCESS);
		}
	}
		...
		memcmp(mKey->data(), key, key_len) != 0)
	{
		...
		if (is_secure_path) {
			if (mDecrypter) {
				CHECK_EQ(WSBSecure_AP4_CencSingleSampleDecrypter_Destroy(mDecrypter), AP4_SUCCESS);
				mDecrypter = NULL;
			}
			CHECK_EQ(WSBSecure_AP4_CencSingleSampleDecrypter_Create(AP4_CENC_ALGORITHM_ID_CTR,
													mKey->data(),
													mKey->size(),
													mBlockCipherFactory,
													mDecrypter), AP4_SUCCESS);
			ALOGV("mDecrypter=%p", mDecrypter);
		} else {
			delete mDecrypter;
	...
	AP4_UI16* plain_sizes_ui16 =
		ReformatArray<size_t, AP4_UI16>((const size_t*)plain_sizes, plain_sizes_len);
	AP4_UI32* encrypted_sizes_ui32 =
		ReformatArray<size_t, AP4_UI32>((const size_t*)enc_sizes, enc_sizes_len);
	...
	void *secure_buf = NULL;
	if (is_secure_path) {
		secure_buf = CdcMemPalloc(mSecureMemOps, buffer->size());
		ap4_out_buffer.SetBuffer((AP4_Byte*)CdcMemGetPhysicAddressCpu(mSecureMemOps, secure_buf), buffer->size());
		result = WSBSecure_AP4_CencSingleSampleDecrypter_DecryptSampleData(mDecrypter,
										   ap4_in_buffer, ap4_out_buffer,
										   (const AP4_UI08*)iv,
    ...
    switch (mode) {
        ...
        case RESUBMIT_BUFFERS:
        {
            if (buffer != NULL && !mCodec->mPortEOS[kPortIndexInput]) {
                ...
                is_video = buffer->meta()->findInt32("nalu-length-size", &nalu_len_size);

				void *secure_buffer = NULL;
				size_t secure_buffer_size = 0; /* 4000+buffer+header */
				size_t secure_data_size = 0;   /* valid buffer+header */
                ...
                if (have_header) {
                    ...
                    sp<RefBase> obj;
					uint8_t *secure_out = NULL;
                    if (buffer->meta()
                        ->findObject("cenc-crypto", &obj)) {

                        if(mCodec->mSecureMemOps == NULL && MARLIN_SECURE_PATH == 1){
                            mCodec->mSecureMemOps = SecureMemAdapterGetOpsS();
                            CdcMemOpen(mCodec->mSecureMemOps);
                        }
                        
                        need_decrypt = true;
                        CHECK(mCodec->DecryptBuffer(buffer, static_cast<MetaData*>(obj.get()), is_video, secure_out));
                    }

					ALOGV("secure_out=%p", secure_out);
                    ...
                    if (buffer->meta()
                        ...
                        if (secure_out) {
                            /* secure path */
                            secure_buffer_size = clear_buffer_size;
                            secure_buffer = CdcMemPalloc(mCodec->mSecureMemOps, secure_buffer_size);
                            CdcMemWrite(mCodec->mSecureMemOps, secure_buffer, header->data(), header->size());
                            uint8_t *phy_secure_out = (uint8_t*)CdcMemGetPhysicAddressCpu(mCodec->mSecureMemOps, secure_out);
                            uint8_t *phy_secure_buffer = (uint8_t*)CdcMemGetPhysicAddressCpu(mCodec->mSecureMemOps, secure_buffer);
                            ALOGV("phy_secure_out=%p, phy_secure_buffer=%p", phy_secure_out, phy_secure_buffer);
                            ret = WSBSecure_PrefixAvcNalus(nalu_len_size,
                                                (uint8_t*)phy_secure_out, buffer->size(),
                                                (uint8_t*)phy_secure_buffer+header->size(),
                                                secure_buffer_size-header->size(),
                                                &prefixed_len);
                            CdcMemPfree(mCodec->mSecureMemOps, secure_out);
                            secure_out = NULL;
                        } else {
                            ret = WSB_PrefixAvcNalus(nalu_len_size,
                                                buffer->data(),
                                                buffer->size(),
                        ...
                        if (secure_buffer) {
                            /* secure path */
                            secure_data_size = header->size() + prefixed_len;
                            clear_buffer->setRange(0, secure_data_size);
                        } else {
                ...
                if (buffer != info->mData) {
                    ...
                    ALOGV("dst=%p, src=%p, size=%d", info->mData->data(), buffer->data(), buffer->size());

                    CHECK_LE(buffer->size(), info->mData->capacity());
                    unsigned int dst = (unsigned int)info->mData->data();
                    /*dst>=0x7c000000&&dst<0x7cf00000*/
                    if (MARLIN_USE_SECUREOS && mCodec->mComponentName.endsWith(".secure")) {
                        ALOGV("secure...");
                        void *vir = CdcMemGetVirtualAddressCpu(mCodec->mSecureMemOps, info->mData->data());
                        if (secure_buffer) {
                            CdcMemCopy(mCodec->mSecureMemOps, vir, secure_buffer, secure_data_size);
                        } else {
                            CdcMemWrite(mCodec->mSecureMemOps, vir, buffer->data(), buffer->size());
                        }
                    } else {
                ...
                if (secure_buffer) {
                    CdcMemPfree(mCodec->mSecureMemOps, secure_buffer);
                    secure_buffer = NULL;
                }
...
bool ACodec::UninitializedState::onAllocateComponent(const sp<AMessage> &msg) {
    ...
    for (size_t matchIndex = 0; matchIndex < matchingCodecs.size();
        ...
        int marlin = 0;
        msg->findInt32("marlin", &marlin);
        ALOGV("marlin=%d", marlin);
#if MARLIN_USE_SECUREOS
        if (marlin) {
            AString tmp = componentName;
            tmp.append(".secure");
            componentName = tmp.c_str();
            if (!mCodec->mSecureInit) {
                mCodec->mSecureInit = true;
                WSBSecure_init();
            }
        }
#endif
        ALOGV("componentName.c_str=%s", componentName.c_str());
