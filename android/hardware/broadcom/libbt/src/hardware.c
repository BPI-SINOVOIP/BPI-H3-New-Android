...
#define FW_PATHFILE_PATH            "/system/vendor/modules/"
...
//Justin Porting 20170103 for New_AP6212 Start
/* AMPAK FW auto detection table */
typedef struct {
    char *chip_id;
    char *updated_chip_id;
} fw_auto_detection_entry_t;
//Justin Porting 20170103 for New_AP6212 End    
...
extern int vnd_used_usb;
extern int baud_rate;
#define UART_BUND_FOR_A80  4000000
/******************************************************************************
**  Static variables
******************************************************************************/
...
//Justin Porting 20170103 for New_AP6212 Start
//static uint8_t hw_config_findpatch(char *p_chip_id_str)
static uint8_t hw_config_findpatch(char *p_chip_id_str, char tmp[])
{
  
    
    //sprintf(p_chip_id_str, "%s", FW_PATHFILE_PATH);
    //strcat(p_chip_id_str, module_info.mod_name);
    //strcat(p_chip_id_str, FW_PATCHFILE_EXTENSION);
    ...
    {
        if (strstr(tmp, p_entry->chip_id) != NULL)
        {
	    
            //strcpy(p_chip_id_str,p_entry->updated_chip_id);
              sprintf(p_chip_id_str, "%s", FW_PATHFILE_PATH);
              strcat(p_chip_id_str,p_entry->updated_chip_id);
              strcat(p_chip_id_str, FW_PATCHFILE_EXTENSION);
			  
              ALOGI("BPI-M2_FW BCM43438A1 patchfile: %s", p_chip_id_str);
    ...
    if (p_buf != NULL)
    {
        ...
        switch (hw_cfg_cb.state)
        {
            case HW_CFG_SET_UART_BAUD_1:
                ...
                ALOGI("bt vendor lib: set UART baud %i", baud_rate);
		    ...
		    if ((status = hw_config_findpatch(p_name, hw_cfg_cb.local_chip_name)) == TRUE)
                    //Justin Porting 20170103 for New_AP6212 End
                    {
...
//#if (BLUETOOTH_HCI_USE_USB == TRUE)
                if (vnd_used_usb == 1)
                {
                    ALOGI("HW_CFG_START--->HCI_READ_LOCAL_NAME");
                    /* read local name */
                    UINT16_TO_STREAM(p, HCI_READ_LOCAL_NAME);
                    *p = 0; /* parameter length */
                    p_buf->len = HCI_CMD_PREAMBLE_SIZE;
