...
int main(int argc, char **argv)
{
   ...
   {
      if ( NULL != memmem(pBuffer,TS_PACKET_SIZE,"VMECM",5)  )
      {
          if ( 0 != memcmp(pBuffer+4,ecmLast+4,TS_PACKET_SIZE-4) )
          {
              result = VMSetECM( pContext, 0, PID(pBuffer), pBuffer, TS_PACKET_SIZE );
              memcpy( ecmLast, pBuffer, TS_PACKET_SIZE );
          }
      }
   }
...
static void usage(char *cmd)
{
   printf("usage %s [-i input_file -m mcastaddr]\n", cmd );
   printf("where:\n");
