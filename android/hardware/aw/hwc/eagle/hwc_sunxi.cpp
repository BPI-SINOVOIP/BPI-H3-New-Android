            ...
            unsigned long long de_freq)
{
    ...
    unsigned int dram_efficience = 60;
...
//	ALOGD("req %llu w %u h %u", de_freq_req, outw, outh);
//	ALOGD("lcd w %llu h %llu", lcdw, lcdh);
...
static int hwc_ui_scaler_check(hwc_layer_1_t *layer, const DisplayInfo *info)
{
	...
	/*
	 * calc the screen width and height
	 */
	int x = calc_point_byPercent(info->PersentWidth, info->InitDisplayWidth >> 1,
			layer->displayFrame.left) * info->VarDisplayWidth / info->InitDisplayWidth;
	int width = calc_point_byPercent(info->PersentWidth, info->InitDisplayWidth >> 1,
			layer->displayFrame.right) * info->VarDisplayWidth / info->InitDisplayWidth;
	width -= x;
	width = (0 == width) ? 1 : width;

	int y = calc_point_byPercent(info->PersentHeight, info->InitDisplayHeight >> 1,
		layer->displayFrame.top) * info->VarDisplayHeight / info->InitDisplayHeight;
	int height = calc_point_byPercent(info->PersentHeight, info->InitDisplayHeight >> 1,
		layer->displayFrame.bottom) * info->VarDisplayHeight / info->InitDisplayHeight;
	height -= y;
	height = (0 == height) ? 1 : height;

    return de_scale_capability_detect(src_w, src_h, width, height,
...
bool match_format(const DisplayInfo *psDisplayInfo, layer_info_t *psHwlayer_info, disp_layer_info *layer_info)
{
    ...
    default:
        ...
        case HAL_PIXEL_FORMAT_YCrCb_420_SP:
            ...
            layer_info->fb.size[1].height = handle->iHeight / 2;
            layer_info->fb.addr[1] = layer_info->fb.addr[0] + ALIGN(
                layer_info->fb.size[0].height * layer_info->fb.size[0].width, 4096);
            layer_info->fb.align[0] = YV12_ALIGN;
    ...
    if(psHwlayer_info->is3D)
    {
        ...
        if(!CheckVideoFormat(handle->iFormat) && (0 == all_3d))
        {
            layer_info->fb.flags = DISP_BF_STEREO_FP;
...
hwc_try_assign_layer(HwcDisContext_t *Localctx, size_t  singcout,int zOrder)
{
    ...
    if((CVBS_USED == Localctx->psDisplayInfo->VirtualToHWDisplay)
	     && !(CheckVideoFormat(handle->iFormat)))
    {
        dueto = D_SKIP_LAYER;
        goto assign_gpu;
    }
    ...
    if (CheckScaleFormat(handle->iFormat) && isValidUiFormat(handle->iFormat)) {
        if (!hwc_ui_scaler_check(psLayer, PsDisplayInfo)) {
            ALOGD("de can not scale ui layer");
            dueto = D_CANNT_SCALE;
            goto assign_gpu;
        }
    }
...
bool sunxi_set(hwc_display_contents_1_t** displays,size_t numDisplays, int *releasefencefd, int *retirefencefd)
{
    ...
    if(Globctx->DetectError == 0 && Globctx->blankAll == 0)
    {
        arg[0] = 0;
    ...
    if(0 != Globctx->exchangeDispChannel)
    {
        if(0 == pthread_mutex_trylock(&Globctx->lock))
        {
			...
			/* hdmi is pullout, disable vsync event */
			if (Globctx->SunxiDisplay[1].DisplayType == DISP_OUTPUT_TYPE_HDMI) {
				arg[0] = Globctx->SunxiDisplay[1].VirtualToHWDisplay;
				arg[1] = 0;
				ioctl(Globctx->DisplayFd, DISP_VSYNC_EVENT_EN,(unsigned long)arg);
				ALOGD("%s disp(%d) type %d vsync force disable!!!\n", __func__, arg[0], Globctx->SunxiDisplay[1].DisplayType);
			}
...
void hwc_destroy_fb()
{
	...
	if(gSunxiHwcDevice.FBFd >= 0) {
		ret =ioctl(gSunxiHwcDevice.FBFd, FBIO_FREE, NULL);
		ALOGV("hwc_destroy_fb: ret=%d", ret);
	}
}
