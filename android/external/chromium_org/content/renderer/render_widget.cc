...
void RenderWidget::OnHandleInputEvent(const WebKit::WebInputEvent* input_event,
  ...
  {
      const WebKit::WebKeyboardEvent* keyboardEvent = static_cast<const WebKit::WebKeyboardEvent*>(input_event);
      if ( (keyboardEvent!=NULL) && (keyboardEvent->type==WebInputEvent::KeyUp) )
      {
          if ( keyboardEvent->windowsKeyCode==ui::VKEY_RETURN )
              OnShowImeIfNeeded();
      }
  }
