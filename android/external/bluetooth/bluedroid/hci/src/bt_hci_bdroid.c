...
extern int brcm_num_hci_cmd_pkts;
extern int rtl_num_hci_cmd_pkts;
static volatile int *num_hci_cmd_pkts;
...
static int init(const bt_hc_callbacks_t* p_cb, unsigned char *local_bdaddr)
{
    ...
    if (module_info.vendor_id == 1) 
    {
        ALOGI("use hci_h5");
        extern tHCI_IF hci_h5_func_table;
        p_hci_if = &hci_h5_func_table;
        num_hci_cmd_pkts = &rtl_num_hci_cmd_pkts;
    }
    else if ((module_info.vendor_id == 0) || (module_info.vendor_id == 2)) 
    {
        ALOGI("use hci_h4");
        extern tHCI_IF hci_h4_func_table;
        p_hci_if = &hci_h4_func_table;
        num_hci_cmd_pkts = &brcm_num_hci_cmd_pkts;
    }
...
static void *bt_hc_worker_thread(void *arg)
{
    ...
    while (lib_running)
    {
        ...
        if (events & HC_EVENT_PRELOAD)
        {
            ...
            {
                if(usb_open(USERIAL_PORT_1) == FALSE)
                    break;
            }
