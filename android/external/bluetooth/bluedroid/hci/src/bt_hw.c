...
void init_vnd_if(unsigned char *local_bdaddr)
{
    ...
    if ((module_info.vendor_id == 0) || (module_info.vendor_id == 2))
    {
        ALOGI("load libbt-vendor-brcm.so");
        dlhandle = dlopen("libbt-vendor-brcm.so", RTLD_NOW);
    }
    else if (module_info.vendor_id == 1)
    {
        ALOGI("load libbt-vendor-rtl.so");
