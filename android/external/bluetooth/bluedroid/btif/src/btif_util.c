...
static int get_bt_addr(char *ptr, char *key, char *mac_value)
{
    char *value = strchr(ptr, '=');
    int name_len = strlen(ptr);
...
int bd_parse_kernel_cmdline(char* key, char* value)
{
    char cmdline[1024];
    char *ptr;
    int fd;
    ...
    if (fd >= 0) {
        ALOGI("open cmdline file successfully...");
        ...
        ALOGI("open cmdline file failed...");
        cmdline[0] = 0;
    }
    ...
    while (ptr && *ptr) {
        char *x = strchr(ptr, ' ');
        if (x != 0) *x++ = 0;
        if (get_bt_addr(ptr, key, value) == 0)
            return 0;
        ptr = x;
    }

    ALOGI("get bda from cmdline failed...");
    return -1;
}
