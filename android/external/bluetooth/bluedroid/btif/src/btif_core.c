...
#define PERSIST_USBBT   "persist.service.bdroid.usbbt"
/************************************************************************************
...
static void btif_fetch_local_bdaddr(bt_bdaddr_t *local_addr)
{
    ...
    if (bd_parse_kernel_cmdline(KERNEL_CMDLINE_BDA_KEY, bta) == 0) {
        BTIF_TRACE_DEBUG0("get bda from cmdline...");

        str2bd(bta, local_addr);
    ...
    BTIF_TRACE_DEBUG0("get bda not from cmdline...");
...
bt_status_t btif_init_bluetooth()
{
    ...
    if (module_info.vendor_id == 2)
        property_set(PERSIST_USBBT, "1");
    else
        property_set(PERSIST_USBBT, "0");
...
bt_status_t btif_enable_bluetooth(void)
{
    ...
    {
        char pval[256];
        property_get("sys.usbbt.inserted", pval, NULL);
        if (pval[0] != '1')
        {
            ALOGI("USB BT not inserted, ignore enable operation...");
