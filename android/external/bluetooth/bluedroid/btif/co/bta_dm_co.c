...
tBTE_APPL_CFG bte_appl_cfg_brcm = { 0x5, 0x4, 0x7, 0x7, 0x10 };
tBTE_APPL_CFG bte_appl_cfg_rtl = { 0x5, 0x4, 0x0, 0x1, 0x10 };
                          ...
                          tBTA_LE_KEY_TYPE  *p_resp_key )
{
    ...
    *p_oob_data = FALSE;
    ...
    {
        if (bte_appl_cfg_rtl.ble_auth_req)
            *p_auth_req = bte_appl_cfg_rtl.ble_auth_req | (bte_appl_cfg_rtl.ble_auth_req & 0x04) | ((*p_auth_req) & 0x04);

        if (bte_appl_cfg_rtl.ble_io_cap <=4)
            *p_io_cap = bte_appl_cfg_rtl.ble_io_cap;

        if (bte_appl_cfg_rtl.ble_init_key<=7)
            *p_init_key = bte_appl_cfg_rtl.ble_init_key;

        if (bte_appl_cfg_rtl.ble_resp_key<=7)
            *p_resp_key = bte_appl_cfg_rtl.ble_resp_key;

        if (bte_appl_cfg_rtl.ble_max_key_size > 7 && bte_appl_cfg_rtl.ble_max_key_size <= 16)
            *p_max_key_size = bte_appl_cfg_rtl.ble_max_key_size;
    }
    else
    {
        if (bte_appl_cfg_brcm.ble_auth_req)
            *p_auth_req = bte_appl_cfg_brcm.ble_auth_req | (bte_appl_cfg_brcm.ble_auth_req & 0x04) | ((*p_auth_req) & 0x04);

        if (bte_appl_cfg_brcm.ble_io_cap <=4)
            *p_io_cap = bte_appl_cfg_brcm.ble_io_cap;

        if (bte_appl_cfg_brcm.ble_init_key<=7)
            *p_init_key = bte_appl_cfg_brcm.ble_init_key;

        if (bte_appl_cfg_brcm.ble_resp_key<=7)
            *p_resp_key = bte_appl_cfg_brcm.ble_resp_key;

        if (bte_appl_cfg_brcm.ble_max_key_size > 7 && bte_appl_cfg_brcm.ble_max_key_size <= 16)
            *p_max_key_size = bte_appl_cfg_brcm.ble_max_key_size;
    }
}
