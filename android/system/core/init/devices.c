...
#ifdef USE_USB_BT
static char usb_bt_devpath[50] = {0};

/* return 1: is usb bt device
   return -1: is not usb bt device
   @path: the path of device node
*/
static int is_usb_bt_device(const char *path)
{
    ...
    if (!strncmp(path, "/dev/bus/usb", strlen("/dev/bus/usb"))) {
        ERROR("open path: %s\n", path);
        ctx = usb_host_init();
        if (!ctx) {
            ERROR("usb_host_init fail...\n");
            return is;
        }
        usbdev = usb_device_open(path);
        if (!usbdev) {
            ERROR("can not open device %s\n", path);
            usb_host_cleanup(ctx);
            return is;
        }
        dev_desc = usb_device_get_device_descriptor(usbdev);
        if ((dev_desc->bDeviceClass == 0xe0) && (dev_desc->bDeviceSubClass == 0x01) && (dev_desc->bDeviceProtocol == 0x01)) {
            is = 1;
            sprintf(usb_bt_devpath, "%s", path);
        } 
        usb_device_close(usbdev);
        usb_host_cleanup(ctx);
    }
...
#define SERVER_NAME    "usb_bt_server"
    ...
    if (socket_fd < 0) {
        ERROR("creat client socket error!!!\n");
        return;
    }
...
#ifdef USE_USB_BT
    // for usb bt adapter
    // we want to make the dev node adaptively instead of writing it into ueventd.$hardware.rc
    // add by liangkangnan@allwinnertech.com
