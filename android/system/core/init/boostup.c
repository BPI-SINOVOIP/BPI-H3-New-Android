...
#define AW_SYSCTRL_LMK_FAST_LEV_MIN     "1"
#define AW_SYSCTRL_LMK_FAST_LEV_LOW     "2"
#define AW_SYSCTRL_LMK_FAST_LEV_HIG		"3"
...
#define AW_BOOST_UP_CPUS_KARAOK         "1008000 3 0 0 1200000 4 0 0 0"
...
static struct boost_list blist = { 0 };
static struct boostup_mode bmode = { 0 };
...
static int detect_update = 1000; //for 1000ms detect update proc info
...
static int aw_system_recreate_cma_area(int index)
{
	...
	fd = open("/dev/ion", O_WRONLY);
	if (fd < 0) {
		ERROR("Failed to open ion device, errno:%s\n", strerror(errno));
		goto err;
	}
	size = (unsigned long)index;
	cdata.cmd = ION_IOC_SUNXI_ADJUST_CMA;
	cdata.arg = (unsigned long)&size;
    ERROR(" Adjust ion memory %dM \n", size);
	ret = ioctl(fd, ION_IOC_CUSTOM, &cdata);
	if (ret < 0){
		ERROR("Failed to ioctl ion device, errno:%s\n", strerror(errno));
		goto err;
	}
	B_LOG("ioctl ion device success! \n");
	return 0;
err:
	return -1;
}
...
static int aw_system_watch_group_memory(int detect_stage, int index)
{
	...
	else if (detect_stage == DETECT_WORK)
	{
		...
		if (sys_detect == USER_ACTIVE)
		{
			...
			int usage = aw_get_process_usage(pid, detect_update);
			if (usage > 30)
			{
				detect_update = 500;  //grep usage per 500 ms
				detect_grap   = 500;  //scan thread per 500 ms
				ligh_cpuload  = 0;
				switch(++high_cpuload)
				{
					B_LOG(" stage %d !\n", high_cpuload);
					case RECLAIM_SERIOUSEST:
						if (dram_size <= 512)
                        {
						...
						break;
					case RECLAIM_SERIOUSER:
						aw_sysctrl_set(AW_SYSCTRL_LMK_FAST_CALL, AW_SYSCTRL_LMK_FAST_LEV_HIG); //kill background stage 3
						aw_sysctrl_set("/proc/sys/vm/drop_caches", "1"); //flsuh cpu-cacge
						break;
					case RECLAIM_SERIOUS:
						aw_sysctrl_set(AW_SYSCTRL_LMK_FAST_CALL, AW_SYSCTRL_LMK_FAST_LEV_LOW); //kill background stage 2
						aw_sysctrl_set("/proc/sys/vm/drop_caches", "1"); //flsuh cpu-cacge
						break;
					case RECLAIM_HIGH:
						aw_sysctrl_set(AW_SYSCTRL_LMK_FAST_CALL, AW_SYSCTRL_LMK_FAST_LEV_MIN); //kill background stage 1
						aw_sysctrl_set("/proc/sys/vm/drop_caches", "1"); //flsuh cpu-cacge
						break;
					case RECLAIM_LOW:
...
static int aw_boost_normal(void)
{
	if (lowmem_limit == 0) {
		...
		if (dram_size <= 512) {
			...
			aw_sysctrl_set("/proc/sys/vm/dirty_background_ratio","2");
			aw_sysctrl_set("/proc/sys/vm/min_free_kbytes","8192");
			aw_sysctrl_set("/sys/block/nandd/queue/read_ahead_kb","32");
			aw_sysctrl_set("/sys/block/nandn/queue/read_ahead_kb","32");
		} else {
			aw_sysctrl_set("/proc/sys/vm/extra_free_kbytes","86400");
			aw_sysctrl_set("/proc/sys/vm/min_free_kbytes","16384");
		}
	...
	if(!strncmp(name, "media.boost.pref", strlen("media.boost.pref"))){
		...
		if(!strncmp(value, "mode", strlen("mode"))){
			switch(value[4]){
				...
				case 'C':
					#ifdef SUN8IW7P1
					aw_system_recreate_cma_area(index);
					#endif
					break;
				case 'd':
				case 'D':
