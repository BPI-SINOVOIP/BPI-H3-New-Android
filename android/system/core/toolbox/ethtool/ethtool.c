              ...
              const struct feature_state *ref_state)
{
    ...
    for (i = 0; i < ARRAY_SIZE(off_flag_def); i++) {
        ...
        if (defs->off_flag_matched[i] == 0 &&
            ...
            kernel_ver < (int)off_flag_def[i].min_kernel_ver)
            continue;
...
static int do_spause(struct cmd_context *ctx)
{
    ...
    struct cmdline_info cmdline_pause[] = {
        { "autoneg", CMDL_BOOL, &pause_autoneg_wanted,
          &epause.autoneg, 0, NULL },
        { "rx", CMDL_BOOL, &pause_rx_wanted, &epause.rx_pause, 0, NULL },
        { "tx", CMDL_BOOL, &pause_tx_wanted, &epause.tx_pause, 0, NULL },
    };
    int err, changed = 0;
...
static int do_sring(struct cmd_context *ctx)
{
    ...
    struct cmdline_info cmdline_ring[] = {
        { "rx", CMDL_S32, &ring_rx_wanted, &ering.rx_pending, 0, NULL },
        { "rx-mini", CMDL_S32, &ring_rx_mini_wanted,
          &ering.rx_mini_pending, 0, NULL },
        { "rx-jumbo", CMDL_S32, &ring_rx_jumbo_wanted,
          &ering.rx_jumbo_pending, 0, NULL },
        { "tx", CMDL_S32, &ring_tx_wanted, &ering.tx_pending, 0, NULL },
    };
    int err, changed = 0;
...
static int do_schannels(struct cmd_context *ctx)
{
    ...
    struct cmdline_info cmdline_channels[] = {
        { "rx", CMDL_S32, &channels_rx_wanted, &echannels.rx_count, 0, NULL },
        { "tx", CMDL_S32, &channels_tx_wanted, &echannels.tx_count, 0, NULL },
        { "other", CMDL_S32, &channels_other_wanted,
          &echannels.other_count, 0, NULL },
        { "combined", CMDL_S32, &channels_combined_wanted,
          &echannels.combined_count, 0, NULL },
    };
    int err, changed = 0;
...
static int do_gregs(struct cmd_context *ctx)
{
    ...
    struct cmdline_info cmdline_gregs[] = {
        { "raw", CMDL_BOOL, &gregs_dump_raw, NULL, 0, NULL },
        { "hex", CMDL_BOOL, &gregs_dump_hex, NULL, 0, NULL },
        { "file", CMDL_STR, &gregs_dump_file, NULL, 0, NULL },
    };
    int err;
    struct ethtool_drvinfo drvinfo;
...
static int do_geeprom(struct cmd_context *ctx)
{
    ...
    int geeprom_length = -1;
    struct cmdline_info cmdline_geeprom[] = {
        { "offset", CMDL_U32, &geeprom_offset, NULL, 0, NULL },
        { "length", CMDL_U32, &geeprom_length, NULL, 0, NULL },
        { "raw", CMDL_BOOL, &geeprom_dump_raw, NULL, 0, NULL },
    };
    int err;
    struct ethtool_drvinfo drvinfo;
...
static int do_seeprom(struct cmd_context *ctx)
{
    ...
    u32 seeprom_magic = 0;
    int seeprom_length = -1;
    u32 seeprom_offset = 0;
    u8 seeprom_value = 0;
    int seeprom_value_seen = 0;
    struct cmdline_info cmdline_seeprom[] = {
        { "magic", CMDL_U32, &seeprom_magic, NULL, 0, NULL },
        { "offset", CMDL_U32, &seeprom_offset, NULL, 0, NULL },
        { "length", CMDL_U32, &seeprom_length, NULL, 0, NULL },
...
static int do_getmodule(struct cmd_context *ctx)
{
    ...
    struct cmdline_info cmdline_geeprom[] = {
        ...
        { "length", CMDL_U32, &geeprom_length, NULL, 0, NULL },
        { "raw", CMDL_BOOL, &geeprom_dump_raw, NULL, 0, NULL },
        { "hex", CMDL_BOOL, &geeprom_dump_hex, NULL, 0, NULL },
    };
...
static int do_seee(struct cmd_context *ctx)
{
    ...
    struct cmdline_info cmdline_eee[] = {
        { "advertise",    CMDL_U32,  &adv_c,       &eeecmd.advertised, 0, NULL },
        { "tx-lpi",       CMDL_BOOL, &lpi_c,   &eeecmd.tx_lpi_enabled, 0, NULL },
        { "tx-timer",      CMDL_U32,  &lpi_time_c, &eeecmd.tx_lpi_timer, 0, NULL },
        { "eee",      CMDL_BOOL, &eee_c,       &eeecmd.eee_enabled, 0, NULL },
    };
...
static const struct option {
    ...
    { "-a|--show-pause", 1, do_gpause, "Show pause options", NULL },
    { "-A|--pause", 1, do_spause, "Set pause options",
      "        [ autoneg on|off ]\n"
      "        [ rx on|off ]\n"
      "        [ tx on|off ]\n" },
    { "-c|--show-coalesce", 1, do_gcoalesce, "Show coalesce options", NULL },
    { "-C|--coalesce", 1, do_scoalesce, "Set coalesce options",
      ...
      "        [tx-frames-high N]\n"
      "        [sample-interval N]\n" },
    { "-g|--show-ring", 1, do_gring, "Query RX/TX ring parameters", NULL },
    ...
    { "-k|--show-features|--show-offload", 1, do_gfeatures,
      "Get state of protocol offload and other features", NULL },
    { "-K|--features|--offload", 1, do_sfeatures,
      "Set protocol offload and other features",
      "        FEATURE on|off ...\n" },
    { "-i|--driver", 1, do_gdrv, "Show driver information", NULL },
    ...
    { "-E|--change-eeprom", 1, do_seeprom,
      ...
      "        [ length N ]\n"
      "        [ value N ]\n" },
    { "-r|--negotiate", 1, do_nway_rst, "Restart N-WAY negotiation", NULL },
    { "-p|--identify", 1, do_phys_id,
      "Show visible port identification (e.g. blinking)",
      "               [ TIME-IN-SECONDS ]\n" },
    { "-t|--test", 1, do_test, "Execute adapter self test",
      "               [ online | offline | external_lb ]\n" },
    { "-S|--statistics", 1, do_gstats, "Show adapter statistics", NULL },
    ...
    { "-T|--show-time-stamping", 1, do_tsinfo,
      "Show time stamping capabilities", NULL },
    { "-x|--show-rxfh-indir", 1, do_grxfhindir,
      "Show Rx flow hash indirection", NULL },
    ...
    { "-P|--show-permaddr", 1, do_permaddr,
      "Show permanent hardware address", NULL },
    ...
    { "-W|--set-dump", 1, do_setfwdump,
      ...
      "        N\n"},
    { "-l|--show-channels", 1, do_gchannels, "Query Channels", NULL },
    { "-L|--set-channels", 1, do_schannels, "Set Channels",
      "               [ rx N ]\n"
      "               [ tx N ]\n"
      "               [ other N ]\n"
      "               [ combined N ]\n" },
    { "--show-priv-flags" , 1, do_gprivflags, "Query private flags", NULL },
    ...
    { "-m|--dump-module-eeprom|--module-info", 1, do_getmodule,
      ...
      "        [ offset N ]\n"
      "        [ length N ]\n" },
    { "--show-eee", 1, do_geee, "Show EEE settings", NULL},
    { "--set-eee", 1, do_seee, "Set EEE settings",
      "        [ eee on|off ]\n"
      "        [ advertise %x ]\n"
      "        [ tx-lpi on|off ]\n"
      "        [ tx-timer %d ]\n"},
    { "-h|--help", 0, show_usage, "Show this help", NULL },
    { "--version", 0, do_version, "Show version number", NULL },
