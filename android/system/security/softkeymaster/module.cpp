...
static bool is_keymaster_sw2hw(void)
{
    char tmp[PROP_VALUE_MAX];

    if (property_get("ro.crypto.sw2hwkeymaster", tmp, NULL)) {
        if (strcmp(tmp, "true") == 0) {
            ALOGD("sw2hwkeymaster is true.");
            return true;
        } else if (strcmp(tmp, "false") == 0) {
            ALOGD("sw2hwkeymaster is false.");
            return false;
        } else {
            ALOGD("sw2hwkeymaster value[%s].", tmp);
            return true;
        }
    }
    ALOGD("not set sw2hwkeymaster.");
    return false;
}
...
static int openssl_open(const hw_module_t* module, const char* name,
    ...
    dev->flags = is_keymaster_sw2hw() ? 0 : KEYMASTER_SOFTWARE_ONLY;
