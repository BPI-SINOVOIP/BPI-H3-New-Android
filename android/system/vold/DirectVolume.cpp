...
DirectVolume::DirectVolume(VolumeManager *vm, int labelIndex,unsigned int devId) :
        Volume(vm, labelIndex,devId) {
    mPaths = new PathCollection();
    for (int i = 0; i < MAX_PARTITIONS; i++)
        mPartMinors[i] = -1;
    mPendingPartMap = 0;
    mDiskMajor = MAJOR(devId);
    mDiskMinor = MINOR(devId);
    mOrigDiskMajor = -1;
    mOrigDiskMinor = -1;
    mDiskNumParts = 0;
    mPartsEventCnt = 0;
    ...
    snprintf(mount, PATH_MAX, "%s/%s%2.2d", Volume::MEDIA_DIR,USB_STORAGE_PATH, labelIndex);
    mMountpoint = strdup(mount);

    snprintf(mount, PATH_MAX, "%s/%s%2.2d", Volume::FUSE_DIR,USB_STORAGE_PATH, labelIndex);
    mFuseMountpoint = strdup(mount);
...
int DirectVolume::handleBlockEvent(NetlinkEvent *evt) {
    ...
    for (it = mPaths->begin(); it != mPaths->end(); ++it) {
        if (!strncmp(dp, *it, strlen(*it))) {
            ...
            if (getDevId()!=0 && MKDEV(major,minor)!=getDevId())
            {
                errno = ENODEV;
                return -1;
            }
            ...
            if (action == NetlinkEvent::NlActionAdd) {
                ...
                if (getState() == Volume::State_Idle||getDevId()!=0) {
                    char msg[255];
...
void DirectVolume::enhanceUsbStability(NetlinkEvent *evt) {
        char path[256];
        ...
        if (tmp && !strncmp(tmp, "sd", 2)) {
           sprintf(path, "/sys/block/%s/events_poll_msecs", tmp);
           usleep(1000 * 1000);
           if ((fp = fopen(path, "r+ "))) {
               if (fprintf(fp, "%d\n", msecs) <= 0) {
