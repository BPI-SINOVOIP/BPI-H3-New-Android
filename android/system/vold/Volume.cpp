...
const char *Volume::USB_STORAGE_PATH = "usbhost/Storage";
...
Volume::Volume(VolumeManager *vm, int labelIndex,unsigned int devId) {
    mVm = vm;
    mDebug = false;

    mLabelIndex = labelIndex;
    char buf[PATH_MAX];
    snprintf(buf, PATH_MAX, "%s%2.2d",USB_STORAGE_PATH ,labelIndex);
    mLabel = strdup(buf);
...
int Volume::deleteMountPoint(char* mountpoint) {
    if (mountpoint) {
        if (mountpoint==getFuseMountpoint() || mountpoint==getMountpoint())
            return 0;
        if (!access(mountpoint, F_OK)) {
...
int Volume::mountVol() {
    ...
    if (primaryStorage!=0) {            // Special case the primary SD card.
        ...
        //not for gms  should always use AID_SDCARD_RW
        aid = AID_SYSTEM;
        gid = AID_SDCARD_RW;
        permMask = 0702;
     }
	...
	// cmcc wasu: 0x08
	if (getBusinessPlatform() == PLATFORM_CMCCWASU)
	{
		gid = AID_SDCARD_RW;
		permMask = 0000;
	}
    ...
    for (i = 0; i < n; i++) {
        ...
        if ((mPartIdx == -1) && (n > 1)) {
            ...
            if (access(mMountPart[i], F_OK)) {
                if (mkdir(mMountPart[i], 0777)) {
                }
            } else {
            }
        }
        ...
        } else if (!mount(devicePath, mMountPart[i],"ext4",
             MS_NOATIME | MS_NODEV | MS_NODIRATIME, "")) {
            chmod(mMountPart[i],0777);
            SLOGE("%s mount via ext4/ext3/ext2 sucess! \n", devicePath);
        } else {
...
int Volume::unmountVol(bool force, bool revert) {
    ...
    char mount[PATH_MAX];
    snprintf(mount, PATH_MAX, "%s/%s", Volume::MEDIA_DIR,USB_STORAGE_PATH);
    if (!strncmp(getMountpoint(),mount,strlen(mount)))
    {
       rmdir(getMountpoint());
    }
    setUuid(NULL);
