...
static int process_config(VolumeManager *vm)
{
    ...
    for (i = 0; i < fstab->num_entries; i++) {
        if (fs_mgr_is_voldmanaged(&fstab->recs[i])) {
            ...
            if (!strncmp(fstab->recs[i].label, "usbhost",7)) {
                SLOGW("**** add devpath %s to vUsbDevPath collection ",fstab->recs[i].blk_device);
                vm->addUsbDevPath(strdup(fstab->recs[i].blk_device));
                continue;
            }
