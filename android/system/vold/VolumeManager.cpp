...
VolumeManager::VolumeManager() {
    ...
    mUsbDevPathCollection = new UsbDevPathCollection();
    ...
    {
        volumeBitmap[i]=false;
    }
    volumeLastCreatedBit=0;
}
...
int VolumeManager::addVolume(unsigned int devId, const char* devPath)
{

    SLOGW("add Volume: dev:%u path:%s",devId,devPath);
    int index = 0;
    while ( volumeBitmap[index] )
    {
        index++;
    ...
    if (index == MAX_VOLUMES)
    {
        SLOGW("full volume! can't create new one");
        return -1;
    }
    else
    {
        volumeBitmap[index]=true;
        index++;
        SLOGW("add volume: get index %d \n",index);
        DirectVolume* dv = new DirectVolume(this, index, devId);
        dv->addPath(devPath);
        mVolumes->push_back(dv);
        return 0;
    }
}
void VolumeManager::removeVolume(unsigned int devId) {
    Volume* toRemove;
    VolumeCollection::iterator it;
    for (it = mVolumes->begin(); it != mVolumes->end(); ++it) {
        if ( (**it).getDevId()==devId )
        {
            toRemove = *it;
            mVolumes->erase(it);
            int emptyLabel = toRemove->getLabelIndex();
            volumeBitmap[emptyLabel-1] = false;
            SLOGW("remove volume %d\n",emptyLabel);
            delete toRemove;
            break;
        }
    }
...
Volume* VolumeManager::makeNewVolume(unsigned int devId, const char* devPath) {
        return NULL;
}
...
void VolumeManager::handleBlockEvent(NetlinkEvent *evt) {
    ...
    unsigned int major = (unsigned int)atoi(evt->findParam("MAJOR"));
    unsigned int minor = (unsigned int)atoi(evt->findParam("MINOR"));
    unsigned int devId = MKDEV(major,minor);
    ...
    if (!strcmp(devtype, "disk"))
    {
         ...
         if (tmp) {
            partitionNum = atoi(tmp);
            SLOGW("disk add uevent majro:%u minor:%u  get NPARTS:",major,minor,partitionNum);
         } else {
            SLOGW("disk add uevent majro:%u minor:%u  miss 'NPARTS'",major,minor);
            partitionNum = -1;
         }
    }

    if (action == NetlinkEvent::NlActionAdd &&
         isUsbDevPath(devpath) ) {
        if (strcmp(devtype, "disk") || partitionNum==0)
        {
        for (it = mVolumes->begin(); it != mVolumes->end(); ++it) {
            if ((*it)->getDevId()==devId && (*it)->getEnable()==Volume::ENABLE)
            {
                hit =true;
                break;
            }
    ...
    else if ( hit && action == NetlinkEvent::NlActionRemove &&
         (strcmp(devtype, "disk") || partitionNum==0) && isUsbDevPath(devpath) ) {
            //removeVolume(devId);
            volume->setEnable(Volume::DISABLE);
    }
}
...
void VolumeManager::updateUsbVolumes() {
    VolumeCollection::iterator i;
    for (i = mVolumes->begin(); i != mVolumes->end(); ++i) {
        if ((*i)->getEnable() == Volume::DISABLE)
            (*i)->setEnable(Volume::RECYCLE);
    }

}

void VolumeManager::recycleUsbVolumes() {
    Volume* toRemove;
    VolumeCollection::iterator i;
    for (i = mVolumes->begin(); i != mVolumes->end();) {
        if ((*i)->getEnable() == Volume::RECYCLE)
        {
            toRemove = *i;
            mVolumes->erase(i++);
            int emptyLabel = toRemove->getLabelIndex();
...
void VolumeManager::addUsbDevPath(char *path) {
    mUsbDevPathCollection->push_back(path);
}

bool VolumeManager::isUsbDevPath(const char *path) {
    UsbDevPathCollection::iterator it;
    for (it = mUsbDevPathCollection->begin(); it != mUsbDevPathCollection->end(); ++it) {
        if ( !strncmp(path,*it,strlen(*it)) )
        {
            return true;
        }
    }
...
void VolumeManager::cleanUsbDevPath() {

}

void VolumeManager::dumpVolumes() {
    VolumeCollection::iterator i;
    for (i = mVolumes->begin(); i != mVolumes->end(); ++i) {
        SLOGW( "Volume %d mountpoint:%s state:%d enable:%d ",
                 (*i)->getLabelIndex(), (*i)->getFuseMountpoint(),
                 (*i)->getState(),(*i)->getEnable() );
    }
}
