    ...
    protected boolean onItemSelected(MenuItem item) {
        ...
        switch (action) {
            ...
            case R.id.action_crop: {
				...
				//------------------------------------------------------------add by fuqiang for crop pic in SD card.
				String scheme = manager.getContentUri(path).getScheme();
				String mPathName = null;
				Log.d("fuqiang","scheme = " + scheme);
				if (scheme.equals("content"))
				{
			    	Cursor c = null;
			        String[] VIDEO_PROJECTION = new String[] { Video.Media.DATA };
					ContentResolver cr = activity.getContentResolver(); 
					c = cr.query(manager.getContentUri(path),VIDEO_PROJECTION,null,null,null);
					if(c != null)
					{
						c.moveToFirst();
				...
				{
					mPathName = manager.getContentUri(path).getPath();
				}
				Intent intent;
				if(!mPathName.startsWith("/mnt/extsd") && !mPathName.startsWith("/mnt/usbhost1"))
				{
				...
				{
					intent = new Intent("com.android.awgallery.CROP");
					//intent.setClass(activity.createPackageContext("com.android.awgallery", 0), com.android.awgallery.app.CropImage.class);
					intent.setComponent(new ComponentName("com.android.awgallery", "com.android.awgallery.app.CropImage"));
