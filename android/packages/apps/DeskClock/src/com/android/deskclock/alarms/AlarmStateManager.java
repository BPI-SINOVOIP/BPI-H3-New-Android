...
public final class AlarmStateManager extends BroadcastReceiver {
    ...
    public static void setFiredState(Context context, AlarmInstance instance) {
        ...
        long enterBootFastTime = pm.getEnterBootFastTime();
        long absTime = Math.abs(currUpTime - enterBootFastTime);

        Log.i("currUpTime        = " + currUpTime);
        Log.i("enterBootFastTime = " + enterBootFastTime);
        Log.i("absTime           = " + absTime);

        if( absTime <= 10000 ){
            Log.i("in boot fast status!");
            setMissedState(context, instance);
        } else {
            Log.i("not in boot fast status!");
