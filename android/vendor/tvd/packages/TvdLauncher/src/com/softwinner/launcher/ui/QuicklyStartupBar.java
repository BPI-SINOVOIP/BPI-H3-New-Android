...
public class QuicklyStartupBar extends RelativeLayout implements View.OnClickListener{
    ...
    public QuicklyStartupBar(Context context, AttributeSet attrs) {
        ...
        if (wm!=null)
        {
            ...
            offset = (screenWidth - (ITEM_NUM-1)*240 - 320)/2;
        }
    }
    ...
    private void setViewForMiddle(){
        ...
        v.setImageResource(mid_image_selected_list[mSelectedPosition%ITEM_NUM]);
        int t = v.getTop();
        int height = v.getHeight();
        int l = mSelectedPosition%ITEM_NUM*240;
        int width = v.getWidth();
        RelativeLayout.LayoutParams lp = (LayoutParams) v.getLayoutParams();
        lp.leftMargin = offset +mSelectedPosition%ITEM_NUM*240;
    ...
    private void setTitle(ArrayList<TextView> text,int [] id){
        ...
        for(int i=0;i<ITEM_NUM;i++){
            ...
            if(i==mSelectedPosition%ITEM_NUM){
                text.get(i).setTextColor(text_selected_color[mSelectedPosition%ITEM_NUM]);
    ...
    private void setImage(ArrayList<ImageView> list,int [] sel,int [] unsel){
        for(int i=0;i<ITEM_NUM;i++){
            if(i==mSelectedPosition%ITEM_NUM){
                ...
                list.get(i).setImageResource(unsel[i%ITEM_NUM]);
            }
        }
    }
    ...
    protected void onFocusChanged(boolean gainFocus, int direction, Rect previouslyFocusedRect) {
        if(gainFocus){
            mText.get(mSelectedPosition%ITEM_NUM).setTextColor(text_selected_color[mSelectedPosition%ITEM_NUM]);
        }else{
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if(keyCode == KeyEvent.KEYCODE_DPAD_LEFT){
            moveToLeft();
        }else if(keyCode == KeyEvent.KEYCODE_DPAD_RIGHT || keyCode == KeyEvent.KEYCODE_TAB){
            moveToRight();
