...
public class EventHandler implements OnClickListener,OnItemSelectedListener,OnItemClickListener, OnItemLongClickListener
{
	...
	private boolean copy_from_samba = false;
    ...
    BroadcastReceiver mNetStateReceiver;
				...
				if (cm != null) {
					...
					if (info != null && info.isConnected()) {
						...
						for(LinkAddress l:link.getLinkAddresses()) {
							if(mLinkAddress == null || !mLinkAddress.equals(l)) {
								mLinkAddress = l;
	...
	private void showPictureMessage(String path) {
		...
		if (thumb != null) {
			...
			Rect rect = getPicResolution(path);
			getPicDetail(path, rect.width(), rect.height());
		} else {
			/* 先设置一个默认图片 */
			imageThumb.setBackgroundResource(R.drawable.thumbnail_bg);
	...
	private Rect getPicResolution(String path){
		// 获取图片分辨率
	...
	private void getPicDetail(String path, int width, int height) {
		// 获取图片大小
...
//			mSamba.umountSmb(currentDir);
	...
	public boolean goIntoDir(String path) {
		File file = new File(path);
		if (file.isDirectory()) {
			if (file.canRead() || (isInSambaMountDir(path) || SmbFileOperate.canRead(file))) {
				currentDir = path;
				saveBrowseDir();
	...
	public boolean isInSambaMountDir() {
		if(mSamba.isSambaMountDir(currentDir)) {
			return true;
		} else {
			return false;
		}
	...
	public boolean isInSambaMountDir(String path) {
		if(mSamba.isSambaMountDir(path)) {
			return true;
		} else {
			return false;
		}
	...
	public boolean isInNfsMode(){
		if(currentDir.equals(MediaProvider.NFS_SHARE) || mNfs.isNfsServer(currentDir) || mNfs.isNfsMountedPoint(currentDir))
			return true;
		return false;
	}
	...
	public void copyFile(String oldLocation, String newLocation) {
		...
		if (!(old.canRead() || (isInSambaMountDir(oldLocation) && SmbFileOperate.canRead(old)))
				|| (delete_after_copy
				&& !(old.canWrite() || (isInSambaMountDir(oldLocation) && SmbFileOperate.canWrite(old))))
				|| !newDir.canWrite()) {
			DisplayToast(mContext.getResources().getString(
					R.string.operate_fail_dueto_permission));
			return;
