...
//    private EthPreference mSelected;
    ...
    private boolean getIpoeState() {
        ...
        try {
            return Settings.Global.getInt(cr, Settings.Global.ETHERNET_USE_IPOE) == 1? true : false;
        } catch (Exception ex) {
            return false;
        }
    }
    ...
    private boolean checkConfigValid(EthernetDevInfo config) {
        try {
            ...
            str = config.getIpAddress();
            if(str.length() == 0) {
                displayToast("IP Address Empty !");
                Log.e(TAG,"IP Address Empty !");
                field.set(mStaticDialog, false);
                return false;
            } else if (!InetAddress.isNumeric(str)) {
            ...
            str = config.getNetMask();
            if(str.length() > 0 && !InetAddress.isNumeric(str)) {
            ...
            str = config.getGateWay();
            if(str.length() > 0 && !InetAddress.isNumeric(str)) {
                displayToast("GateWay is invalid !");
                Log.e(TAG,"GateWay is invalid !");
                field.set(mStaticDialog, false);
                return false;
            }
            ...
            str = config.getDns1();
            if(str.length() > 0 && !InetAddress.isNumeric(str)) {
                displayToast("Dns1 is invalid !");
                Log.e(TAG,"Dns1 is invalid !");
                field.set(mStaticDialog, false);
                return false;
            }
            ...
            str = config.getDns2();
            if(str.length() > 0 && !InetAddress.isNumeric(str)) {
                displayToast("Dns2 is invalid !");
                Log.e(TAG,"Dns2 is invalid !");
                field.set(mStaticDialog, false);
                return false;
            }
    ...
    public void onClick(DialogInterface dialog, int whichbutton) {
        ...
        if (whichbutton == DialogInterface.BUTTON_POSITIVE) {
            ...
            if(currentMode.equals(DHCP_MODE)) {
                ...
                if (!checkConfigValid(configDevInfo)) {
                    return;
                }
