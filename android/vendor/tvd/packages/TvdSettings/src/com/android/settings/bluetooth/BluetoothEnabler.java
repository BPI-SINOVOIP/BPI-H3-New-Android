...
public final class BluetoothEnabler implements CompoundButton.OnCheckedChangeListener {
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        ...
        /* If USB Dongle is used, BT can not be enable before USB Dongle is inserted. */
        if (isChecked) {
            if (SystemProperties.getInt("persist.service.bdroid.usbbt", 0) == 1) {
                if (SystemProperties.getInt("sys.usbbt.inserted", 0) == 0) {
                    Toast.makeText(mContext, "Please insert USB Dongle first.", Toast.LENGTH_LONG).show();
                    buttonView.setChecked(false);
                    return;
                }
            }
