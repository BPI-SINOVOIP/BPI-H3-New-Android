...
static CmccScreenRecoveryUI* self = NULL;
...
CmccScreenRecoveryUI::CmccScreenRecoveryUI() :
    currentIcon(NONE),
    installingFrame(0),
    rtl_locale(false),
...
void CmccScreenRecoveryUI::draw_install_overlay_locked(int frame) {
...
// Should only be called with updateMutex locked.
void CmccScreenRecoveryUI::draw_background_locked(Icon icon)
{
    pagesIdentical = false;
    gr_color(0, 0, 0, 255);
...
// Should only be called with updateMutex locked.
void CmccScreenRecoveryUI::draw_progress_locked()
{
...
void CmccScreenRecoveryUI::SetColor(UIElement e) {
    switch (e) {
        case HEADER:
...
// Should only be called with updateMutex locked.
void CmccScreenRecoveryUI::draw_screen_locked()
{
...
// Should only be called with updateMutex locked.
void CmccScreenRecoveryUI::update_screen_locked()
{
    draw_screen_locked();
    gr_flip();
}
...
// Should only be called with updateMutex locked.
void CmccScreenRecoveryUI::update_progress_locked()
{
...
void* CmccScreenRecoveryUI::progress_thread(void *cookie) {
    self->progress_loop();
    return NULL;
}

void CmccScreenRecoveryUI::progress_loop() {
...
void CmccScreenRecoveryUI::LoadBitmap(const char* filename, gr_surface* surface) {
...
void CmccScreenRecoveryUI::LoadLocalizedBitmap(const char* filename, gr_surface* surface) {
...
void CmccScreenRecoveryUI::Init()
{
    gr_init();
...
void CmccScreenRecoveryUI::SetLocale(const char* locale) {
    if (locale) {
...
void CmccScreenRecoveryUI::SetBackground(Icon icon)
{
    pthread_mutex_lock(&updateMutex);
...
void CmccScreenRecoveryUI::SetProgressType(ProgressType type)
{
    pthread_mutex_lock(&updateMutex);
...
void CmccScreenRecoveryUI::ShowProgress(float portion, float seconds)
{
    pthread_mutex_lock(&updateMutex);
...
void CmccScreenRecoveryUI::SetProgress(float fraction)
{
    pthread_mutex_lock(&updateMutex);
...
void CmccScreenRecoveryUI::Print(const char *fmt, ...)
{
    char buf[256];
    va_list ap;
    va_start(ap, fmt);
...
void CmccScreenRecoveryUI::StartMenu(const char* const * headers, const char* const * items,
...
int CmccScreenRecoveryUI::SelectMenu(int sel) {
    int old_sel;
    pthread_mutex_lock(&updateMutex);
    if (show_menu > 0) {
        old_sel = menu_sel;
...
void CmccScreenRecoveryUI::EndMenu() {
    int i;
    pthread_mutex_lock(&updateMutex);
...
bool CmccScreenRecoveryUI::IsTextVisible()
{
    pthread_mutex_lock(&updateMutex);
...
bool CmccScreenRecoveryUI::WasTextEverVisible()
{
    pthread_mutex_lock(&updateMutex);
...
void CmccScreenRecoveryUI::ShowText(bool visible)
{
    pthread_mutex_lock(&updateMutex);
    show_text = visible;
...
void CmccScreenRecoveryUI::Redraw()
{
    pthread_mutex_lock(&updateMutex);
    update_screen_locked();
    pthread_mutex_unlock(&updateMutex);
...
int* CmccScreenRecoveryUI::GetScreenPara()
{
